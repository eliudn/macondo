...
public partial class sedesincluyeninnovacionpei : System.Web.UI.Page
{
    ...
    public void obtenerGET()
    {
        ...
        subtitulo.Text = "Indicador: Sedes educativas que incluyen innovación en los énfasis formativos de sus PEI.";
        descripcion.Text = "";
  ...
  for (int i = 0; i < datos.Rows.Count; i++)
    ...
    [WebMethod(EnableSession = true)]
    public static string sedesincluyeninnovacionpei_(string respuesta, string opc)
    {
        ...
        DataTable sedesincluyeninnovacionpei = lb.detalleInstitucionesincluyenxRespuesta("", "", "", respuesta1, opc1);


        if (sedesincluyeninnovacionpei != null && sedesincluyeninnovacionpei.Rows.Count > 0)
        {
            int total = sedesincluyeninnovacionpei.Rows.Count;
            ca = "lleno@";
            for (int i = 0; i < sedesincluyeninnovacionpei.Rows.Count; i++)
            {
                var f = i + 1;
                ca += "<tr>";
                ca += "<td><b>" + f + ".</b> </td>";
                ca += "<td>" + sedesincluyeninnovacionpei.Rows[i]["nombredepartamento"].ToString() + " </td>";
                ca += "<td>" + sedesincluyeninnovacionpei.Rows[i]["nombremunicipio"].ToString() + " </td>";
                ca += "<td>" + sedesincluyeninnovacionpei.Rows[i]["dane"].ToString() + " - " + sedesincluyeninnovacionpei.Rows[i]["nombreins"].ToString() + " </td>";
                ca += "</tr>";
            }
            ca += "@" + total;
        }
    ...
    [WebMethod(EnableSession = true)]
    public static string realizarBusquedasedesincluyeninnovacionpei_(string coddepartamento, string codmunicipio, string codinstitucion, string respuesta, string opc)
    {
        string ca = "";
        var lb = new LineaBase();

        DataTable sedesincluyeninnovacionpei = lb.detalleInstitucionesincluyenxRespuesta(coddepartamento, codmunicipio, codinstitucion, respuesta, opc);
