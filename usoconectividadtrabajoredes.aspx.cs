...
public partial class usoconectividadtrabajoredes : System.Web.UI.Page
{
    ...
    public void obtenerGET()
    {
        ...
        subtitulo.Text = "Indicador: Uso de conectividad para el trabajo de las redes temáticas.";
        descripcion.Text = "";
    ...
    [WebMethod(EnableSession = true)]
    public static string usoconectividadtrabajoredes_(string respuesta)
    {
        ...
        DataTable usoconectividadtrabajoredes = lb.detalleUsoconectividadtrabajoredesxFrecuencia("", "", "", "", respuesta1);


        if (usoconectividadtrabajoredes != null && usoconectividadtrabajoredes.Rows.Count > 0)
        {
            int total = usoconectividadtrabajoredes.Rows.Count;
            ca = "lleno@";
            for (int i = 0; i < usoconectividadtrabajoredes.Rows.Count; i++)
            {
                var f = i + 1;
                ca += "<tr>";
                ca += "<td><b>" + f + ".</b> </td>";
                ca += "<td>" + usoconectividadtrabajoredes.Rows[i]["nombredepartamento"].ToString() + " </td>";
                ca += "<td>" + usoconectividadtrabajoredes.Rows[i]["nombremunicipio"].ToString() + " </td>";
                ca += "<td>" + usoconectividadtrabajoredes.Rows[i]["dane"].ToString() + " - " + usoconectividadtrabajoredes.Rows[i]["nombreins"].ToString() + " </td>";
                ca += "<td>" + usoconectividadtrabajoredes.Rows[i]["danesede"].ToString() + " - " + usoconectividadtrabajoredes.Rows[i]["nombresede"].ToString() + " </td>";
                ca += "<td>" + usoconectividadtrabajoredes.Rows[i]["respuesta"].ToString() + " </td>";
                ca += "</tr>";
            }
            ca += "@" + total;
        }
    ...
    [WebMethod(EnableSession = true)]
    public static string realizarBusquedausoconectividadtrabajoredes_(string coddepartamento, string codmunicipio, string codinstitucion, string codsede, string respuesta)
    {
        string ca = "";
        var lb = new LineaBase();

        DataTable usoconectividadtrabajoredes = lb.detalleUsoconectividadtrabajoredesxFrecuencia(coddepartamento, codmunicipio, codinstitucion, codsede, respuesta);
