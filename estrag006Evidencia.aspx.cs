...
public partial class estrag006Evidencia : System.Web.UI.Page
{
    ...
    public void obtenerGET()
    {
        ...
        lblCodU.Text = HttpUtility.UrlDecode(HttpContext.Current.Request.QueryString["u"]);
    }
    ...
    protected void btnActualizarDatosTutoria_Click(object sender, EventArgs e)
    {
        ...
        else if (variables[0] == "exito")
        {
            ...
            if (usu.agregarArchivoRespositorioEntregaMaterial(lblCodUsuario.Text, variables[2], variables[3], variables[4], variables[5], variables[7], Convert.ToInt32(variables[8]), fun.getFechaAñoHoraActual(), lblCodInstrumento.Text))
                {
                    gvCargarEvidencias();
    ...
    private string[] subirArchivo(FileUpload trepador)
    {
        ...
        if (HttpContext.Current.Request.ApplicationPath.Remove(0, 1) == "")
            pathsinmap = "~/EntregaMaterial/";
        else
            pathsinmap = "~/EntregaMaterial/" + HttpContext.Current.Request.ApplicationPath.Remove(0, 1) + "/";
    ...
    {
        DataTable datos = est.cargarEvidenciasEntregaMaterial(lblCodInstrumento.Text, lblCodUsuario.Text);
    ...
    {
        DataTable datos = est.cargarEvidenciasEntregaMaterialI(lblCodInstrumento.Text);
        GridEvidencias.DataSource = datos;
        GridEvidencias.DataBind();
    ...
    {
        Estrategias usu = new Estrategias();
        DataRow dato = usu.buscarEvidenciaEntregaMaterial(cod);
        if (usu.borrarEvidenciaEntregaMaterial(cod))
        {
    ...
    {
        if (lblCodU.Text != "")//Asesor Unimag
        {
            Response.Redirect("estrag006reporte2.aspx");
        }
        else
        {
