...
public partial class docentemasivoredt : System.Web.UI.Page
{
             ...
             int validarID = 0;
              ...
              if (nit != null && nit != "")
              {
                  ...
                  if(validarGenero == 0 || validarTDoc == 0 || validarID == 0)//Carga los docentes
                  {
                      ...
                      string codred = HttpUtility.HtmlDecode(GridView1.Rows[gvr.RowIndex].Cells[11].Text);
                      ...
                      if(datDocente == null)
                      {
                          if (doc.agregarDocente(nomDoc, ApeDoc, idDoc, genero, fun.convertFechaAño(fnac), tel, dir, email, tipodoc["cod"].ToString()))
                          {
                              ...
                              if (datoSede != null)
                              {
                                  if (doc.agregarDocenteGrado(idDoc, datoSede["codigo"].ToString(), codanio))
                                  {
                                      ...
                                      if (bdocente != null)
                                      {
                                          est.agregarDocentexRedTematica(bdocente["cod"].ToString(), codred);
                                      }
                          ...
                          {
                             //El docente si existe pero no está matriculado
                              DataRow datoSede = ins.buscarSedexNit(nit);
                              if (datoSede != null)
                              {
                                  DataRow bdocente = ins.buscarDocenteMatriculado(idDoc, datoSede["codigo"].ToString(), codanio);
                                  ...
                                  if (bdocente == null)
                                  {
                                      if (doc.agregarDocenteGrado(idDoc, datoSede["codigo"].ToString(), codanio))
                                      {
                                          ...
                                          est.agregarDocentexRedTematica(bdocente["cod"].ToString(), codred);
                                          ca += "Docente ya matriculado -> " + idDoc + " y matriculado a la red " + codred + "<br/>";
                                      }
                                  }
                                  else
                                  {
                                      DataRow docentert = est.buscarDocenteRedTematica(bdocente["cod"].ToString(), codred);
                                      if (docentert == null)
                                      {
                                          est.agregarDocentexRedTematica(bdocente["cod"].ToString(), codred);
                                          ca += "Docente Editado -> " + idDoc + " en sede con DANE: " + nit + " y matriculado a la red " + codred + "<br/>";
                                      }
                                      else
                                      {
                                          ca += "Docente ya matriculado -> " + idDoc + " en sede con DANE: " + nit + " y ya está matriculado a la red " + codred + "<br/>";
                                      }
                                      malos++;
                      ...
                      else
                      {
                           ...
                           if (datoSede != null)
                           {
                               ...
                               else
                               {
                                   ...
                                   {
                                       ca += "Docente Editado -> " + idDoc + " en sede con DANE: " + nit + " y ya está matriculado a la red " + codred + "<br/>";
                                   }
