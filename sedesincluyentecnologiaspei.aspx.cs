...
public partial class sedesincluyentecnologiaspei : System.Web.UI.Page
{
    ...
    public void obtenerGET()
    {
        ...
        subtitulo.Text = "Indicador: Sedes educativas que incluyen tecnología en los énfasis formativos de sus PEI.";
        descripcion.Text = "";
  ...
  for (int i = 0; i < datos.Rows.Count; i++)
    ...
    [WebMethod(EnableSession = true)]
    public static string sedesincluyentecnologiaspei_(string respuesta, string opc)
    {
        ...
        DataTable sedesincluyentecnologiaspei = lb.detalleInstitucionesincluyenxRespuesta("", "", "", respuesta1, opc1);


        if (sedesincluyentecnologiaspei != null && sedesincluyentecnologiaspei.Rows.Count > 0)
        {
            int total = sedesincluyentecnologiaspei.Rows.Count;
            ca = "lleno@";
            for (int i = 0; i < sedesincluyentecnologiaspei.Rows.Count; i++)
            {
                var f = i + 1;
                ca += "<tr>";
                ca += "<td><b>" + f + ".</b> </td>";
                ca += "<td>" + sedesincluyentecnologiaspei.Rows[i]["nombredepartamento"].ToString() + " </td>";
                ca += "<td>" + sedesincluyentecnologiaspei.Rows[i]["nombremunicipio"].ToString() + " </td>";
                ca += "<td>" + sedesincluyentecnologiaspei.Rows[i]["dane"].ToString() + " - " + sedesincluyentecnologiaspei.Rows[i]["nombreins"].ToString() + " </td>";
                ca += "</tr>";
            }
            ca += "@" + total;
        }
    ...
    [WebMethod(EnableSession = true)]
    public static string realizarBusquedasedesincluyentecnologiaspei_(string coddepartamento, string codmunicipio, string codinstitucion, string respuesta, string opc)
    {
        string ca = "";
        var lb = new LineaBase();

        DataTable sedesincluyentecnologiaspei = lb.detalleInstitucionesincluyenxRespuesta(coddepartamento, codmunicipio, codinstitucion, respuesta, opc);
