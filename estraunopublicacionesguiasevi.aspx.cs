...
public partial class estraunopublicacionesguiasevi : System.Web.UI.Page
{
    ...
    {
        Session["t"] = HttpUtility.UrlDecode(HttpContext.Current.Request.QueryString["t"]);
        Session["sg"] = HttpUtility.UrlDecode(HttpContext.Current.Request.QueryString["sg"]);
        lblTipo.Text = Session["t"].ToString();
        if (lblTipo.Text != "")
        {
            switch (Session["t"].ToString())
            {
                case "1":
                    lbltitulo.Text = "Guías reimpresas";
                    Panel2.Visible = false;
                    Panel3.Visible = false;
                    PanelMomento0.Visible = true;
                    break;
                case "2":
                    lbltitulo.Text = "Publicaciones impresas y/o digitales";
                     Panel2.Visible = true;
                    Panel3.Visible = false;
                    PanelMomento0.Visible = false;
                    break;
                case "3":
                    lbltitulo.Text = "Guía de investigación rediseñada";
                     Panel2.Visible = false;
                    Panel3.Visible = true;
                    PanelMomento0.Visible = false;
                    break;
    ...
    {
        Response.Redirect("menusegestrategia1.aspx");
    }
    protected void btnActualizarDatosTutoria_Click(object sender, EventArgs e)
    {
        ...
        else if (variables[0] == "exito")
        {
            ...
            if (usu.agregarArchivoRespositorioPublicaciones(lblCodUsuario.Text, variables[2], variables[3], variables[4], variables[5], variables[7], Convert.ToInt32(variables[8]), fun.getFechaAñoHoraActual(), rbtActividades.SelectedValue, lblTipo.Text))
            {
                gvCargarEvidencias();
    ...
    {
        DataTable datos = est.cargarEvidenciasPublicaciones(lblTipo.Text);
    ...
    {
        Estrategias usu = new Estrategias();
        DataRow dato = usu.buscarEvidenciaPublicaciones(cod);
        if (usu.borrarEvidenciaPublicaciones(cod))
        {
