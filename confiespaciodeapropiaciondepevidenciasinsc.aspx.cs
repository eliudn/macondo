...
public partial class confiespaciodeapropiacionevidenciasinsc : System.Web.UI.Page
{
    ...
    {
        Response.Redirect("confiespaciodeapropiaciondep.aspx");
    }
    protected void btnActualizarDatosTutoria_Click(object sender, EventArgs e)
    {
        ...
        else if (variables[0] == "exito")
        {
            ...
            if (est.agregarArchivoRespositorioEspacioxDepartamento(lblCodUsuario.Text, variables[2], variables[3], variables[4], variables[5], variables[7], Convert.ToInt32(variables[8]), fun.getFechaAñoHoraActual(), lblCodEspacio.Text, rbtActividades.SelectedValue))
            {
                //gvCargarEvidencias();
    ...
    private string[] subirArchivo(FileUpload trepador)
    {
        ...
        if (HttpContext.Current.Request.ApplicationPath.Remove(0, 1) == "")
            pathsinmap = "~/Ferias_departamentales/";
        else
            pathsinmap = "~/Ferias_departamentales/" + HttpContext.Current.Request.ApplicationPath.Remove(0, 1) + "/";
    ...
    {
        DataTable datos = est.cargarEvidenciasEspacioxDepartamento(lblCodEspacio.Text, rbtActividadesBuscar.SelectedValue, lblCodUsuario.Text);
    ...
    private void borrarImagen(string cod)
    {
        ...
        DataRow dato = usu.buscarEvidenciaEspacioxDepartamento(cod);
        if (usu.borrarEvidenciaEspacioxDepartamento(cod))
        {
