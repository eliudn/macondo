...
public partial class usoconectividadformacionestudintes : System.Web.UI.Page
{
    ...
    public void obtenerGET()
    {
        ...
        subtitulo.Text = "Indicador: Uso de la conectividad en la formación de maestros.";
        descripcion.Text = "";
    ...
    [WebMethod(EnableSession = true)]
    public static string usoconectividadformacionmaestros_(string respuesta)
    {
        ...
        DataTable usoconectividadformacionmaestros = lb.detalleUsoconectividadformacionestudiantesxFrecuencia("", "", "", "", respuesta);


        if (usoconectividadformacionmaestros != null && usoconectividadformacionmaestros.Rows.Count > 0)
        {
            int total = usoconectividadformacionmaestros.Rows.Count;
            ca = "lleno@";
            for (int i = 0; i < usoconectividadformacionmaestros.Rows.Count; i++)
            {
                var f = i + 1;
                ca += "<tr>";
                ca += "<td><b>" + f + ".</b> </td>";
                ca += "<td>" + usoconectividadformacionmaestros.Rows[i]["nombredepartamento"].ToString() + " </td>";
                ca += "<td>" + usoconectividadformacionmaestros.Rows[i]["nombremunicipio"].ToString() + " </td>";
                ca += "<td>" + usoconectividadformacionmaestros.Rows[i]["dane"].ToString() + " - " + usoconectividadformacionmaestros.Rows[i]["nombreins"].ToString() + " </td>";
                ca += "<td>" + usoconectividadformacionmaestros.Rows[i]["danesede"].ToString() + " - " + usoconectividadformacionmaestros.Rows[i]["nombresede"].ToString() + " </td>";
                ca += "<td>" + usoconectividadformacionmaestros.Rows[i]["respuesta"].ToString() + " </td>";
                ca += "</tr>";
            }
            ca += "@" + total;
        }
    ...
    [WebMethod(EnableSession = true)]
    public static string realizarBusquedausoconectividadformacionmaestros_(string coddepartamento, string codmunicipio, string codinstitucion, string codsede, string respuesta)
    {
        string ca = "";
        var lb = new LineaBase();

        DataTable usoconectividadformacionmaestros = lb.detalleUsoconectividadformacionmaestrosxFrecuencia(coddepartamento, codmunicipio, codinstitucion, codsede, respuesta);
