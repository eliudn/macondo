...
public partial class detalleSedesImpacto : System.Web.UI.Page
{
    ...
    public void obtenerGET()
    {
        ...
        titulo.Text = "Sedes que participaron en la evaluacion Impacto";
        subtitulo.Text = "Indicador: Total de sede que diligenciado la evaluacion Impacto. ";
        descripcion.Text = "";
  ...
  for (int i = 0; i < datos.Rows.Count; i++)
    ...
    [WebMethod(EnableSession = true)]
    public static string detalleImpacto_(string respuesta)
    {
        detalleSedesImpacto d = new detalleSedesImpacto();
        string ca = "";
        ...
        DataTable instituEvaluacionInter = lb.detalleSedesImpacto();

        if (instituEvaluacionInter != null && instituEvaluacionInter.Rows.Count > 0)
        {
            int total = instituEvaluacionInter.Rows.Count;
            ca = "lleno@";
            for (int i = 0; i < instituEvaluacionInter.Rows.Count; i++)
            {
                var f = i + 1;
                ca += "<tr>";
                ca += "<td><b>" + f + ".</b> </td>";
                ca += "<td>" + instituEvaluacionInter.Rows[i]["nombredepartamento"].ToString() + " </td>";
                ca += "<td>" + instituEvaluacionInter.Rows[i]["nombremunicipio"].ToString() + " </td>";
                ca += "<td>" + instituEvaluacionInter.Rows[i]["daneins"].ToString() + " - " + instituEvaluacionInter.Rows[i]["nombrein"].ToString() + " </td>";
                ca += "<td>" + instituEvaluacionInter.Rows[i]["danesede"].ToString() + " - " + instituEvaluacionInter.Rows[i]["monbresede"].ToString() + " </td>";
    ...
    [WebMethod(EnableSession = true)]
    public static string busquedaInstitucionIntermedia_(string coddepartamento, string codmunicipio, string codinstitucion)
    {
        string ca = "";
        var lb = new LineaBase();
        DataTable instituEvaluacionInter = lb.busquedadetalleSede(coddepartamento, codmunicipio, codinstitucion);
