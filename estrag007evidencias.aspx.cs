...
public partial class estrag007evidencias : System.Web.UI.Page
{
    ...
    {
        lblCodestrag007.Text = HttpUtility.UrlDecode(HttpContext.Current.Request.QueryString["codestrag007"]);
    ...
    protected void btnRegresar_click(object sender, EventArgs e)
    {
        ...
        {
            Response.Redirect("estrag007Reporte.aspx?e=" + Session["e"].ToString() + "&m=" + Session["m"].ToString() + "&s=" + Session["s"].ToString() + "&a=" + Session["a"].ToString());
        }
        else if (Session["codrol"].ToString() == "11")
        {
            Response.Redirect("estrag007.aspx?e=" + Session["e"].ToString() + "&m=" + Session["m"].ToString() + "&s=" + Session["s"].ToString());     
        }
    ...
    protected void btnActualizarDatosTutoria_Click(object sender, EventArgs e)
    {
        ...
        else if (variables[0] == "exito")
        {
            ...
            if (usu.agregarArchivoRespositorioEstrategia1Asesorg007(lblCodUsuario.Text, variables[2], variables[3], variables[4], variables[5], variables[7], Convert.ToInt32(variables[8]), fun.getFechaAñoHoraActual(),  rbtActividades.SelectedValue, lblCodestrag007.Text))
            {
                gvCargarEvidencias();
    ...
    {
        DataTable datos = est.cargarEvidenciasEstrategia1Asesorg007(lblCodestrag007.Text);
    ...
    {
        Estrategias usu = new Estrategias();
        DataRow dato = usu.buscarEvidenciaEstrategia1g007(cod);
        if (usu.borrarEvidenciaEstrategia1g007(cod))
        {
