...
public partial class intermediainstrurectores : System.Web.UI.Page
{
    string codrol = "";
    ...
    protected void dropCiudad_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        {
            ddCiudadxDepartamento(DropDownList2, DropDownList1.SelectedValue);
        }
    }
    ...
    private void cargarInfoIE(string dane, DataRow datoinsasesor)
    {
        ...
        if(datoie != null)
        {
            ...
            DropDownList2.SelectedValue = datoie["codmunicipio"].ToString();

            DataRow dpto = inst.buscarDepartamentoxCiudad(datoie["codmunicipio"].ToString());
            if(dpto != null)
                DropDownList1.SelectedValue = dpto["cod"].ToString();

            DropDownList3.SelectedValue = datoie["codzona"].ToString();
    ...
    protected void btnIniciarInfoBasica_Click(object sender, EventArgs e)
    {
            ...
            if (lblCodRol.Text == "7")
            {
                ...
                ddCiudad(DropDownList2);
                ddDepartamentos(DropDownList1);
                ddZona(DropDownList3);
    ...
    protected void imgVer_Click(object sender, ImageClickEventArgs e)
    {
        ...
        DropDownList3.SelectedValue = codzona;
    ...
    protected void btnPrimerGuardar_Onclick(object sender, EventArgs e)
    {
        ...
        if (datoie != null)
        {
            ...
            if (dato != null)
            {
                        ...
                        if (inst.actualizarDatosInstitucion(txtDANE.Text, txtNomInstitucion.Text, dropNomRector.SelectedValue, txtDireccion.Text, txtTelefono.Text, txtFax.Text, txtemail.Text, txtWeb.Text, DropDownList2.SelectedValue, DropDownList3.SelectedValue, dropPropiedadJuridica.SelectedValue, txtActivas.Text, txtInactivas.Text))
                        {
            ...
            else
            {
                        ...
                        {
                            lblCodInstAsesor.Text = codinstasesor["codigo"].ToString();
                            if (inst.actualizarDatosInstitucion(txtDANE.Text, txtNomInstitucion.Text, dropNomRector.SelectedValue, txtDireccion.Text, txtTelefono.Text, txtFax.Text, txtemail.Text, txtWeb.Text, dropMunicipio.SelectedValue, DropDownList3.SelectedValue, dropPropiedadJuridica.SelectedValue, txtActivas.Text, txtInactivas.Text))
                            {
                                mostrarmensaje("exito", "Datos Actualizados exitosamente.");
                            }
                            else
        ...
        if (datos != null && datos.Rows.Count > 0)
        {
            ...
            for (int i = 0; i < datos.Rows.Count; i++)
            {
                if (datoie != null)
                {
                    ...
                    {
                        ca += "<option value='" + datoie["codigo"].ToString() + "' selected>" + datos.Rows[i]["nombre"].ToString() + "</option>";
                        val = "1";
                    }
                    else
                    {
