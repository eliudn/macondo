...
public partial class estracincoevidenciaslineamientos : System.Web.UI.Page
{
    ...
    protected void btnActualizarDatosTutoria_Click(object sender, EventArgs e)
    {
        ...
        else if (variables[0] == "exito")
        {
            ...
            //if (dato == null)
            //{
            if (usu.agregarArchivoRespositorioEstra5_Lineamientos(lblCodUsuario.Text, variables[2], variables[3], variables[4], variables[5], variables[7], Convert.ToInt32(variables[8]), fun.getFechaAñoHoraActual(),  rbtActividades.SelectedValue, "5"))
                {
                    gvCargarEvidencias();
    ...
    private string[] subirArchivo(FileUpload trepador)
    {
        ...
        if (HttpContext.Current.Request.ApplicationPath.Remove(0, 1) == "")
            pathsinmap = "~/Estrategia_5/";
        else
            pathsinmap = "~/Estrategia_5/" + HttpContext.Current.Request.ApplicationPath.Remove(0, 1) + "/";
    ...
    {
        DataTable datos = est.cargarEvidenciasEstrategia5_Lineamientos(lblCodUsuario.Text, "5");
    ...
    {
        DataTable datos = est.cargarEvidenciasEstrategia5_Lineamientos("5");
        GridEvidencias.DataSource = datos;
        GridEvidencias.DataBind();
    ...
    {
        Estrategias usu = new Estrategias();
        DataRow dato = usu.buscarEvidenciaEstrategia5(cod);
        if (usu.borrarEvidenciaEstrategia5(cod))
        {
