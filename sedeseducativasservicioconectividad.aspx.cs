...
public partial class sedeseducativasservicioconectividad : System.Web.UI.Page
{
    ...
    public void obtenerGET()
    {
        ...
        subtitulo.Text = "Indicador: Sedes educativas beneficiadas con servicio de conectividad entregado por el Proyecto.";
        descripcion.Text = "";
  ...
  for (int i = 0; i < datos.Rows.Count; i++)
    ...
    [WebMethod(EnableSession = true)]
    public static string sedeseducativasservicioconectividad_(string respuesta)
    {
        ...
        DataTable sedeseducativasservicioconectividad = lb.detallesedeseducativasservicioconectividad("", "", "", "", respuesta1);


        if (sedeseducativasservicioconectividad != null && sedeseducativasservicioconectividad.Rows.Count > 0)
        {
            int total = sedeseducativasservicioconectividad.Rows.Count;
            ca = "lleno@";
            for (int i = 0; i < sedeseducativasservicioconectividad.Rows.Count; i++)
            {
                var f = i + 1;
                ca += "<tr>";
                ca += "<td><b>" + f + ".</b> </td>";
                ca += "<td>" + sedeseducativasservicioconectividad.Rows[i]["nombredepartamento"].ToString() + " </td>";
                ca += "<td>" + sedeseducativasservicioconectividad.Rows[i]["nombremunicipio"].ToString() + " </td>";
                ca += "<td>" + sedeseducativasservicioconectividad.Rows[i]["dane"].ToString() + " - " + sedeseducativasservicioconectividad.Rows[i]["nombreins"].ToString() + " </td>";
                ca += "<td>" + sedeseducativasservicioconectividad.Rows[i]["danesede"].ToString() + " - " + sedeseducativasservicioconectividad.Rows[i]["nombresede"].ToString() + " </td>";
                ca += "<td>" + sedeseducativasservicioconectividad.Rows[i]["respuesta"].ToString() + " </td>";
                ca += "</tr>";
            }
            ca += "@" + total;
        }
    ...
    [WebMethod(EnableSession = true)]
    public static string realizarBusquedasedeseducativasservicioconectividad_(string coddepartamento, string codmunicipio, string codinstitucion, string codsede, string respuesta)
    {
        string ca = "";
        var lb = new LineaBase();

        DataTable sedeseducativasservicioconectividad = lb.detallesedeseducativasservicioconectividad(coddepartamento, codmunicipio, codinstitucion, codsede, respuesta);
