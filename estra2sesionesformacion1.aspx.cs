...
public partial class estra2sesionesformacion1 : System.Web.UI.Page
{
            ...
            if (Session["m"].ToString() == "1")
            {
                if (Session["s"].ToString() == null)
                {
                    if (Session["s"].ToString() != "1")
                    {
                        ...
                        subtitulo.Text = "Sesiones de formación No. 1 realizadas ";
            ...
            if (Session["m"].ToString() == "3")
            {
                if (Session["s"].ToString() != null)
                {
                    if (Session["s"].ToString() == "2")
                    {
                        ...
                        subtitulo.Text = "Sesiones de formación No. 2 realizadas ";
                        descripcion.Text = "G001: Asistencia a eventos";
                    }
                }
            }
                ...
                if (Session["m"].ToString() == "4")
                {
                    if (Session["s"].ToString() != null)
                    {
                        if (Session["s"].ToString() == "3")
                        {
                            ...
                            subtitulo.Text = "Sesiones de formación No. 3 realizadas  ";
                            descripcion.Text = "G001: Asistencia a eventos ";
                        }
                    }
                }
    ...
    [WebMethod(EnableSession = true)]
    public static string estra2sesionesformacion1_()
    {
        string ca = "";
        Estrategias est = new Estrategias();

        DataTable estra2sesionesformacion1 = est.estra2SesionesFormacion1("", "", "", "", HttpContext.Current.Session["m"].ToString(), HttpContext.Current.Session["s"].ToString());


        if (estra2sesionesformacion1 != null && estra2sesionesformacion1.Rows.Count > 0)
        {
            int total = estra2sesionesformacion1.Rows.Count;
            ca = "lleno@";
            for (int i = 0; i < estra2sesionesformacion1.Rows.Count; i++)
            {
                var f = i + 1;
                ca += "<tr>";
                ca += "<td><b>" + f + ".</b> </td>";
                ca += "<td>" + estra2sesionesformacion1.Rows[i]["nombredepartamento"].ToString() + " </td>";
                ca += "<td>" + estra2sesionesformacion1.Rows[i]["nombremunicipio"].ToString() + " </td>";
                ca += "<td>" + estra2sesionesformacion1.Rows[i]["dane"].ToString() + " - " + estra2sesionesformacion1.Rows[i]["nombreins"].ToString() + " </td>";
                ca += "<td>" + estra2sesionesformacion1.Rows[i]["danesede"].ToString() + " - " + estra2sesionesformacion1.Rows[i]["nombresede"].ToString() + " </td>";
                ca += "<td>" + estra2sesionesformacion1.Rows[i]["nombresesion"].ToString() + " </td>";
                ca += "</tr>";
            }
            ca += "@" + total;
        }
     ...
     [WebMethod(EnableSession = true)]
    public static string realizarBusquedaestra2sesionesformacion1_(string coddepartamento, string codmuncipio, string codinstitucion, string codsede)
    {
        string ca = "";
        Estrategias est = new Estrategias();

        DataTable estra2sesionesformacion1 = est.estra2SesionesFormacion1(coddepartamento, codmuncipio, codinstitucion, codsede, HttpContext.Current.Session["m"].ToString(), HttpContext.Current.Session["s"].ToString());
