...
public partial class confiespaciodeapropiacionevidencias : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            if (Session["codrol"] != null)
            {
                usuariointer();
                ObtenerGet();
                //ddMunicipios(DropMunicipiosCargar,lblCodDepartamento.Text);
                //ddMunicipios(DropMunicipiosBuscar, lblCodDepartamento.Text);
                //gvCargarEvidencias();
            }
            else
            {
    ...
    //{
    //    drop.DataSource = estu.cargarmunicipiosxFerias(coddepartamento);
    ...
    protected void btnActualizarDatosTutoria_Click(object sender, EventArgs e)
    {
        ...
        else if (variables[0] == "exito")
        {
            ...
            if (est.agregarArchivoRespositorioEspacioxMunicipio(lblCodUsuario.Text, variables[2], variables[3], variables[4], variables[5], variables[7], Convert.ToInt32(variables[8]), fun.getFechaAñoHoraActual(), lblCodEspacio.Text, "0", rbtActividades.SelectedValue))
            {
                //gvCargarEvidencias();
    ...
    {
        DataTable datos = est.cargarEvidenciasEspacioxMunicipio(lblCodEspacio.Text, "0", rbtActividadesBuscar.SelectedValue, lblCodUsuario.Text);
    ...
    private void usuariointer()
    {
        Usuario usu = new Usuario();
        DataRow dato = usu.usuariointer(Session["codusuario"].ToString());
        if (dato != null)
        {
