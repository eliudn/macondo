...
public partial class sedesincluyeninvestigacionpei : System.Web.UI.Page
{
    ...
    public void obtenerGET()
    {
        ...
        titulo.Text = "Acceso de los estudiantes y maestros a la conectividad entregadas por el Proyecto a las sedes educativas beneficiadas";
        subtitulo.Text = "Indicador: Sedes educativas que incluyen investigación en los énfasis formativos de sus PEI.";
        descripcion.Text = "";
  ...
  for (int i = 0; i < datos.Rows.Count; i++)
    ...
    [WebMethod(EnableSession = true)]
    public static string sedesincluyeninvestigacionpei_(string respuesta, string opc)
    {
        ...
        var opc1 = HttpUtility.UrlDecode(opc);

        DataTable sedesincluyeninvestigacionpei = lb.detalleInstitucionesincluyenxRespuesta("", "", "", respuesta1, opc1);


        if (sedesincluyeninvestigacionpei != null && sedesincluyeninvestigacionpei.Rows.Count > 0)
        {
            int total = sedesincluyeninvestigacionpei.Rows.Count;
            ca = "lleno@";
            for (int i = 0; i < sedesincluyeninvestigacionpei.Rows.Count; i++)
            {
                var f = i + 1;
                ca += "<tr>";
                ca += "<td><b>" + f + ".</b> </td>";
                ca += "<td>" + sedesincluyeninvestigacionpei.Rows[i]["nombredepartamento"].ToString() + " </td>";
                ca += "<td>" + sedesincluyeninvestigacionpei.Rows[i]["nombremunicipio"].ToString() + " </td>";
                ca += "<td>" + sedesincluyeninvestigacionpei.Rows[i]["dane"].ToString() + " - " + sedesincluyeninvestigacionpei.Rows[i]["nombreins"].ToString() + " </td>";
                ca += "</tr>";
            }
            ca += "@" + total;
        }
    ...
    [WebMethod(EnableSession = true)]
    public static string realizarBusquedasedesincluyeninvestigacionpei_(string coddepartamento, string codmunicipio, string codinstitucion, string respuesta, string opc)
    {
        string ca = "";
        var lb = new LineaBase();

        DataTable sedesincluyeninvestigacionpei = lb.detalleInstitucionesincluyenxRespuesta(coddepartamento, codmunicipio, codinstitucion, respuesta, opc);
