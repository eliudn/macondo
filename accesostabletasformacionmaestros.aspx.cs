...
public partial class accesostabletasformacionmaestros : System.Web.UI.Page
{
    ...
    public void obtenerGET()
    {
        ...
        subtitulo.Text = "Indicador: Acceso de las tabletas en la formación de maestros.";
        descripcion.Text = "";
  ...
  for (int i = 0; i < datos.Rows.Count; i++)
    ...
    [WebMethod(EnableSession = true)]
    public static string accesostabletasformacionmaestros_(string respuesta)
    {
        ...
        DataTable accesostabletasformacionmaestros = lb.detalleAccesostabletasxFrecuencia("", "", "", "", respuesta1);


        if (accesostabletasformacionmaestros != null && accesostabletasformacionmaestros.Rows.Count > 0)
        {
            int total = accesostabletasformacionmaestros.Rows.Count;
            ca = "lleno@";
            for (int i = 0; i < accesostabletasformacionmaestros.Rows.Count; i++)
            {
                var f = i + 1;
                ca += "<tr>";
                ca += "<td><b>" + f + ".</b> </td>";
                ca += "<td>" + accesostabletasformacionmaestros.Rows[i]["nombredepartamento"].ToString() + " </td>";
                ca += "<td>" + accesostabletasformacionmaestros.Rows[i]["nombremunicipio"].ToString() + " </td>";
                ca += "<td>" + accesostabletasformacionmaestros.Rows[i]["dane"].ToString() + " - " + accesostabletasformacionmaestros.Rows[i]["nombreins"].ToString() + " </td>";
                ca += "<td>" + accesostabletasformacionmaestros.Rows[i]["danesede"].ToString() + " - " + accesostabletasformacionmaestros.Rows[i]["nombresede"].ToString() + " </td>";
                ca += "<td>" + accesostabletasformacionmaestros.Rows[i]["respuesta"].ToString() + " </td>";
                ca += "</tr>";
            }
            ca += "@" + total;
        }
    ...
    [WebMethod(EnableSession = true)]
    public static string realizarBusquedaaccesostabletasformacionmaestros_(string coddepartamento, string codmunicipio, string codinstitucion, string codsede, string respuesta)
    {
        string ca = "";
        var lb = new LineaBase();

        DataTable accesostabletasformacionmaestros = lb.detalleAccesostabletasxFrecuencia(coddepartamento, codmunicipio, codinstitucion, codsede, respuesta);
