...
public partial class institucionespromueveusoticdocentes : System.Web.UI.Page
{
    ...
    public void obtenerGET()
    {
        ...
        subtitulo.Text = "Indicador: Instituciones educativas que desde el PEI promueve uso de las TIC en los docentes.";
        descripcion.Text = "";
  ...
  for (int i = 0; i < datos.Rows.Count; i++)
    ...
    [WebMethod(EnableSession = true)]
    public static string institucionespromueveusoticdocentes_(string respuesta)
    {
        ...
        DataTable institucionespromueveusoticdocentes = lb.detalleinstucionesPromueveUsoTicDocentesxRespuesta("", "", "", respuesta1);


        if (institucionespromueveusoticdocentes != null && institucionespromueveusoticdocentes.Rows.Count > 0)
        {
            int total = institucionespromueveusoticdocentes.Rows.Count;
            ca = "lleno@";
            for (int i = 0; i < institucionespromueveusoticdocentes.Rows.Count; i++)
            {
                var f = i + 1;
                ca += "<tr>";
                ca += "<td><b>" + f + ".</b> </td>";
                ca += "<td>" + institucionespromueveusoticdocentes.Rows[i]["nombredepartamento"].ToString() + " </td>";
                ca += "<td>" + institucionespromueveusoticdocentes.Rows[i]["nombremunicipio"].ToString() + " </td>";
                ca += "<td>" + institucionespromueveusoticdocentes.Rows[i]["dane"].ToString() + " - " + institucionespromueveusoticdocentes.Rows[i]["nombreins"].ToString() + " </td>";
                ca += "</tr>";
            }
            ca += "@" + total;
        }
    ...
    [WebMethod(EnableSession = true)]
    public static string realizarBusquedainstitucionespromueveusoticdocentes_(string coddepartamento, string codmunicipio, string codinstitucion, string respuesta)
    {
        string ca = "";
        var lb = new LineaBase();

        DataTable institucionespromueveusoticdocentes = lb.detalleinstucionesPromueveUsoTicDocentesxRespuesta(coddepartamento, codmunicipio, codinstitucion, respuesta);
