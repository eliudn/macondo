        ...
        if (!IsPostBack)
        {
            ...
            if (Session["codigo"] != null)
            {
                ddMunicipios(dropMunicipiosSedes, "20");
                    DataRow dat = est.buscarFeriaDepartamentalxcodigo(Session["codigo"].ToString());
            ...
            else
            {
                ...
                lblError.Text = "Error al crear o ver Feria Departamental";
            }
...
//        ScriptManager.RegisterStartupScript(this, typeof(Page), "alerta", script, false);
    ...
    protected void btnAgregarEstudiantes_Click(object sender, EventArgs e)
    {
        ...
        bool feriam = est.actualizarFeriaDepartamental(dropDepartamento.SelectedValue, nombreferiamunicipal.Text, numeroasistentes.Text, numerogrupos.Text, numerodocentes.Text, fechaelaboracion.Text, horaferia.SelectedValue, horaferiafinal.SelectedValue, fechaelaboracioncierre.Text, horaferiacierre.SelectedValue, horaferiafinalcierre.SelectedValue, Session["codigo"].ToString());

        string script = @"<script type='text/javascript'>$('#CrearRed').hide(),$('#VerRed').fadeIn(500),alert('Feria Departamental actualizada exitosamente.');
                     </script>";
