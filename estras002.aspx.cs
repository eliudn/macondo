...
public partial class estras002 : System.Web.UI.Page
{
    ...
    /** Cargar asesores a lista desplegable  sin el seleccione asesor */
    /**  */
        [WebMethod(EnableSession=true)]
    public static string cargarAsesores2()
    {
        string ca = "";
    ...
    [WebMethod(EnableSession = true)]
    public static string listarInstrumentos002()
    {
        Institucion inst = new Institucion();
        string ca = "";
        Funciones fun = new Funciones();

        DataTable datos = inst.listarInstrumentos002(HttpContext.Current.Session["CodAsesorCoordinador"].ToString(), HttpContext.Current.Session["e"].ToString(), HttpContext.Current.Session["m"].ToString(), HttpContext.Current.Session["s"].ToString());
        if (datos != null && datos.Rows.Count > 0)
        {
            for (int i = 0; i < datos.Rows.Count; i++)
            {
                ...
                ca += "<td style='padding:5px;text-align:center;' ><a class='btn btn-success' onclick=\"$('#evidencias').hide(),$('#table').hide(), $('#form').fadeIn(500), loadSelectInstrumentos002(" + datos.Rows[i]["codProyecto"].ToString() + "),cargarInstrumentos002(" + datos.Rows[i]["codigo"].ToString() + "),listaractividadess002(" + datos.Rows[i]["codigo"].ToString() + "),listarcompromisoss002(" + datos.Rows[i]["codigo"].ToString() + ") \">Editar</a><br/ ><br/ ><a href='estras002evidencias.aspx?codestras002=" + datos.Rows[i]["codigo"].ToString() + "' class='btn btn-primary'>Evidencias</a><br/ ><br/ ><a href='javascript:void(0)' onclick='deleteInstrumentos002(" + datos.Rows[i]["codigo"].ToString() + ")' class='btn btn-danger'>Eliminar</a></td>";
                ca += "</tr>";
            }
        }
        else
        {
            ...
            ca += "<td>" + HttpContext.Current.Session["CodAsesorCoordinador"].ToString() + "</td>";
        }
    ...
    public static string matricularDocentes(string codestras002, string codgradodocente)
    {
        ...
        for (int i = 0; i < codgdocente.Length; i++)
        {
            if (i != 0)
            {
                ...
                if (response)
                {
                    ca = "matricula@Se matricularon " + (codgdocente.Length - 2) + " docentes exitosamente";
                }
                else
                {
                    ca += "vacio@Error al matricular los docentes" + codgradodocente;
                }
            }
