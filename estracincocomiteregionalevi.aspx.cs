...
public partial class estracincocomiteregionalevi : System.Web.UI.Page
{
    ...
    protected void btnActualizarDatosTutoria_Click(object sender, EventArgs e)
    {
        ...
        else if (variables[0] == "exito")
        {
            ...
            if (inst.agregarArchivoComiteRegional(lblCodUsuario.Text, variables[2], variables[3], variables[4], variables[5], variables[7], Convert.ToInt32(variables[8]), fun.getFechaAñoHoraActual(), lblCodInstrumento.Text, rbtActividades.SelectedValue))
            {
                gvCargarEvidencias();
    ...
    {
        DataTable datos = inst.cargarEvidenciasComiteRegional(lblCodInstrumento.Text, lblCodUsuario.Text);
    ...
    {
        DataRow dato = inst.buscarEvidenciaComiteRegional(cod);
        if (inst.borrarEvidenciaComiteRegional(cod))
        {
    ...
    {
        Response.Redirect("estracincocomiteregional.aspx");
    }
