...
public partial class bitacora4_1 : System.Web.UI.Page
{
    ...
    public static string insertbitacora4_1(string grupoinvestigacion, string firmamaestro, string firmanino, string fechadiligenciamiento, string firmaasesor)
    {
        ...
        Institucion inst = new Institucion();
        string ca = "";
        DataRow validate = inst.buscarinsertbitacora4_1(grupoinvestigacion, HttpContext.Current.Session["CodAsesorCoordinador"].ToString(), HttpContext.Current.Session["d"].ToString());

        if (validate == null)
        {
            DataRow insert = inst.procinsertbitacora4_1(grupoinvestigacion, firmamaestro, firmanino, fun.convertFechaAño(fechadiligenciamiento), firmaasesor, HttpContext.Current.Session["d"].ToString(), HttpContext.Current.Session["CodAsesorCoordinador"].ToString());
            if (insert != null)
            {
                ca = "true@";
        ...
        {
            ca = "Este Grupo ya tiene una bitácora creada para este desembolso bitacora4_1@";
        }
    ...
    [WebMethod(EnableSession = true)]
    public static string listarBitacora4_1()
    {
        Institucion inst = new Institucion();
        Funciones fun = new Funciones();
        string ca = "";

        DataTable datos = inst.proclistarBitacora4_1(HttpContext.Current.Session["CodAsesorCoordinador"].ToString(), HttpContext.Current.Session["d"].ToString());
        if (datos != null && datos.Rows.Count > 0)
        {
            ...
            for (int i = 0; i < datos.Rows.Count; i++)
            {
                ...
                ca += "<td style='padding:5px;' ><a class='btn btn-success' onclick=\"$('#table').hide(), $('#form').fadeIn(500), cargarBitacora4_1(" + datos.Rows[i]["codigo"].ToString() + ") \">Editar</a><br/><br/><a class='btn btn-danger' onclick=\"eliminar(" + datos.Rows[i]["codigo"].ToString() + ") \">Eliminar</a></td>"; //,  loadSelectBitacoraTres(" + datos.Rows[i]["codProyecto"].ToString() + "), traerPreguntasTres(" + datos.Rows[i]["codigo"].ToString() + "
                ca += "</tr>";
            }
        }
        else
    ...
    [WebMethod(EnableSession = true)]
    public static string loadRubrosBitacora4_1(string rubro, string codigoinstrumento, int total)
    {
        //string ca = "";
