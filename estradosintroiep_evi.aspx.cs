...
public partial class estradosintroiep_evi : System.Web.UI.Page
{
    ...
    protected void btnActualizarDatosTutoria_Click(object sender, EventArgs e)
    {
        ...
        else if (variables[0] == "exito")
        {
            ...
            if (inst.agregarArchivoinstroiepestrados(lblCodUsuario.Text, variables[2], variables[3], variables[4], variables[5], variables[7], Convert.ToInt32(variables[8]), fun.getFechaAñoHoraActual(), lblCodInstrumento.Text, rbtActividades.SelectedValue))
            {
                gvCargarEvidencias();
    ...
    {
        DataTable datos = inst.cargarEvidenciasinstroiepestrados(lblCodInstrumento.Text, lblCodUsuario.Text);
    ...
    private void borrarImagen(string cod)
    {
        ...
        DataRow dato = inst.buscarEvidenciainstroiepestrados(cod);
        if (inst.borrarEvidenciainstroiepestrados(cod))
        {
    ...
    {
        Response.Redirect("estradosintroiep.aspx");
    }
