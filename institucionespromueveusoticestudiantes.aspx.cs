...
public partial class institucionespromueveusoticestudiantes : System.Web.UI.Page
{
    ...
    public void obtenerGET()
    {
        ...
        subtitulo.Text = "Indicador: Instituciones educativas que desde el PEI promueve uso de las TIC en los estudiantes.";
        descripcion.Text = "";
  ...
  for (int i = 0; i < datos.Rows.Count; i++)
    ...
    [WebMethod(EnableSession = true)]
    public static string institucionespromueveusoticestudiantes_(string respuesta)
    {
        ...
        DataTable institucionespromueveusoticestudiantes = lb.detalleinstucionesPromueveUsoTicEstudiantesxRespuesta("", "", "", respuesta1);


        if (institucionespromueveusoticestudiantes != null && institucionespromueveusoticestudiantes.Rows.Count > 0)
        {
            int total = institucionespromueveusoticestudiantes.Rows.Count;
            ca = "lleno@";
            for (int i = 0; i < institucionespromueveusoticestudiantes.Rows.Count; i++)
            {
                var f = i + 1;
                ca += "<tr>";
                ca += "<td><b>" + f + ".</b> </td>";
                ca += "<td>" + institucionespromueveusoticestudiantes.Rows[i]["nombredepartamento"].ToString() + " </td>";
                ca += "<td>" + institucionespromueveusoticestudiantes.Rows[i]["nombremunicipio"].ToString() + " </td>";
                ca += "<td>" + institucionespromueveusoticestudiantes.Rows[i]["dane"].ToString() + " - " + institucionespromueveusoticestudiantes.Rows[i]["nombreins"].ToString() + " </td>";
                ca += "</tr>";
            }
            ca += "@" + total;
        }
    ...
    [WebMethod(EnableSession = true)]
    public static string realizarBusquedainstitucionespromueveusoticestudiantes_(string coddepartamento, string codmunicipio, string codinstitucion, string respuesta)
    {
        string ca = "";
        var lb = new LineaBase();

        DataTable institucionespromueveusoticestudiantes = lb.detalleinstucionesPromueveUsoTicEstudiantesxRespuesta(coddepartamento, codmunicipio, codinstitucion, respuesta);
