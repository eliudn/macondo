...
public partial class confimesadetrabajo : System.Web.UI.Page
{
    ...
    protected void btnAgregarEstudiantes_Click(object sender, EventArgs e)
    {
        if ( Session["codasesorestrategia"] != null)//Valida que la tabla Seleccionar estudiantes este llena
        {
            ...
            DataRow grupo = estr.agregarMesadeTrabajoxSedeDocente(dropSedes.SelectedValue, fun.getFechaAñoHoraActual(), Session["codasesorestrategia"].ToString(), txtNomGrupoInvestigacion.Text);

            if (grupo != null)
            {
                Session["codmesadetrabajo"] = grupo["codigo"].ToString();
               
                foreach (GridViewRow row2 in GridSeleccionDocentes.Rows)
                {
                    string codgradodocente = HttpUtility.HtmlDecode(row2.Cells[0].Text);
                    if (estr.agregarDocentexSedexMesdeTrabajo(codgradodocente, grupo["codigo"].ToString()))
                    {
                        valDoc++;
                    }
                }
            ...
            if (valDoc > 0)
            {
                ...
                string script = @"<script type='text/javascript'> 
                alert('Mesa de Trabajo creado exitosamente.');
    ...
    private void cargarGruposxAsesor()
    {
        ...
        DataTable datos = estr.cargarMesadeTrabajoxAsesor(Session["codasesorestrategia"].ToString());
    ...
    protected void imgVerDocentes_Click(object sender, ImageClickEventArgs e)
    {
        ...
        DataTable datos = estr.cargarDocentesxSedexMesadeTrabajo(codproyectosede);
    ...
    protected void lnkEditarGrupo_Click(object sender, EventArgs e)
    {
        ...
        Session["codmesadetrabajo"] = codgrupoinvestigacion;

        Response.Redirect("confimesadetrabajoeditar.aspx");
    }
    ...
    protected void lnkEliminarGrupo_Click(object sender, EventArgs e)
    {
        ...
        estr.eliminaMesadeTrabajoDocente(codgrupo);
        estr.eliminaMesadeTrabajo(codgrupo);
        estr.eliminaMesadeTrabajoEvidencias(codgrupo);
    ...
    protected void lnkInstrumentoS003_Click(object sender, EventArgs e)
    {
        LinkButton btndetails = sender as LinkButton;
        GridViewRow gvrow = (GridViewRow)btndetails.NamingContainer;
        string codmesadetrabajo = HttpUtility.HtmlDecode(gvrow.Cells[1].Text);

        Session["codmesadetrabajo"] = codmesadetrabajo;
