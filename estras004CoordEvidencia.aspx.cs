...
public partial class estras004CoordEvidencia : System.Web.UI.Page
{
    ...
    public void obtenerGET()
    {
        ...
        lblCodUsuario.Text = HttpUtility.UrlDecode(HttpContext.Current.Request.QueryString["cu"]);
    ...
    protected void btnActualizarDatosTutoria_Click(object sender, EventArgs e)
    {
        ...
        else if (variables[0] == "exito")
        {
            ...
            if (usu.agregarArchivoRespositorioEstrategiaS004Sedes(lblCodUsuario.Text, variables[2], variables[3], variables[4], variables[5], variables[7], Convert.ToInt32(variables[8]), fun.getFechaAñoHoraActual(), lblCodInstrumento.Text, rbtActividades.SelectedValue))
                {
                    gvCargarEvidencias();
    ...
    {
        DataTable datos = est.cargarEvidenciasEstrategia4AsesorRedTematicaCoord(lblCodInstrumento.Text, lblCodUsuario.Text);
    ...
    {
        Estrategias usu = new Estrategias();
        DataRow dato = usu.buscarEvidenciaEstrategiaS004Sedes(cod);
        if (usu.borrarEvidenciaEstrategiaS004Sedes(cod))
        {
    ...
    {
        Response.Redirect("estras004coordinador.aspx?e=" + Session["e"].ToString() + "&m=" + Session["m"].ToString() + "&s=" + Session["s"].ToString());
        //if (Session["codrol"].ToString() == "2" || Session["codrol"].ToString() == "9")//Coordinador y Supervisor CUC
