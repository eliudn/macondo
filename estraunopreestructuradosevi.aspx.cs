...
public partial class estraunopreestructuradosevi : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            if (Session["codrol"] != null)
            {
                buscarUsuario();
                ObtenerGet();
                //aqui crear metodo que busque en la BD mando lo que tenga (lblCodigo.Text)
                //busca en la tabla que carga el grid de (estraunopreestructurados.aspx) y casi el mismo metodo, agregale el WHERE cod='78' por ejemplo
                //si me hago entender?
                //si :V
                //listo.
                //gracias padre!
    ...
    {
        lblCodigo.Text = HttpUtility.UrlDecode(HttpContext.Current.Request.QueryString["cod"]);
        lblCoord.Text = HttpUtility.UrlDecode(HttpContext.Current.Request.QueryString["c"]);
        if (lblCoord.Text != "")
        {
    ...
    protected void btnRegresar_click(object sender, EventArgs e)
    {
        if (lblCoord.Text != "")
        {
            Response.Redirect("estraunopreestructuradoscoord.aspx?e=" + Session["e"].ToString() + "&m=" + Session["m"].ToString());
        }
        else
        {
            Response.Redirect("estraunopreestructurados.aspx?e=" + Session["e"].ToString() + "&m=" + Session["m"].ToString());
        }
    ...
    protected void btnActualizarDatosTutoria_Click(object sender, EventArgs e)
    {
        ...
        else if (variables[0] == "exito")
        {
            ...
            if (usu.agregarArchivoRespositorioEstrategia1Preestructurados(lblCodUsuario.Text, variables[2], variables[3], variables[4], variables[5], variables[7], Convert.ToInt32(variables[8]), fun.getFechaAñoHoraActual(), rbtActividades.SelectedValue, lblCodigo.Text))
    ...
    {
        DataTable datos = est.cargarEvidenciasEstrategia1AsesorPreestructurados(lblCodigo.Text);
    ...
    {
        Estrategias usu = new Estrategias();
        DataRow dato = usu.buscarEvidenciaEstrategia1Preestructurados(cod);
        if (usu.borrarEvidenciaEstrategia1Preestructurados(cod))
        {
