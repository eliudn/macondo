...
public partial class estradospresupuestodesembolsoevi : System.Web.UI.Page
{
    ...
    protected void btnActualizarDatosTutoria_Click(object sender, EventArgs e)
    {
        ...
        else if (variables[0] == "exito")
        {
            ...
            if (inst.agregarArchivoPresupuestoDesembolso(lblCodUsuario.Text, variables[2], variables[3], variables[4], variables[5], variables[7], Convert.ToInt32(variables[8]), fun.getFechaAñoHoraActual(), lblCodInstrumento.Text, rbtActividades.SelectedValue))
            {
                gvCargarEvidencias();
    ...
    {
        DataTable datos = inst.cargarEvidenciasPresupuestoDesembolso(lblCodInstrumento.Text, lblCodUsuario.Text);
    ...
    {
        DataTable datos = inst.cargarEvidenciasPresupuestoDesembolsoI(lblCodInstrumento.Text);
        GridEvidencias.DataSource = datos;
        GridEvidencias.DataBind();
    ...
    private void borrarImagen(string cod)
    {
        ...
        DataRow dato = inst.buscarEvidenciaPresupuestoDesembolsoestrados(cod);
        if (inst.borrarEvidenciaPresupuestoDesembolsoestrados(cod))
        {
    ...
    {
        Response.Redirect("estradospresupuestodesembolso.aspx");
    }
