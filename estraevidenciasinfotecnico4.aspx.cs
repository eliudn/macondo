...
public partial class estraevidenciasinfotecnico4 : System.Web.UI.Page
{
        ...
        if (!IsPostBack)
        {
            if (Session["codrol"] != null)
            {
                ...
                if (Session["codrol"].ToString() == "20")
                {
                    ...
                    this.GridEvidencias.Columns[10].Visible = false;
                    gvCargarEvidenciasI();
                }
                else
                {
    ...
    protected void btnActualizarDatosTutoria_Click(object sender, EventArgs e)
    {
        ...
        else if (variables[0] == "exito")
        {
            ...
            if (usu.agregarArchivoRespositorioInfoTecnico(lblCodUsuario.Text, variables[2], variables[3], variables[4], variables[5], variables[7], Convert.ToInt32(variables[8]), fun.getFechaAñoHoraActual(), lblEstrategia.Text, rbtActividades.SelectedValue))
                {
                    gvCargarEvidencias();
    ...
    private string[] subirArchivo(FileUpload trepador)
    {
        ...
        if (HttpContext.Current.Request.ApplicationPath.Remove(0, 1) == "")
            pathsinmap = "~/Informe_Tecnico/";
        else
            pathsinmap = "~/Informe_Tecnico/" + HttpContext.Current.Request.ApplicationPath.Remove(0, 1) + "/";
    ...
    {
        DataTable datos = est.cargarEvidenciasInfoTecnico(lblEstrategia.Text, lblCodUsuario.Text);
    ...
    {
        DataTable datos = est.cargarEvidenciasInfoTecnico(lblEstrategia.Text);
        GridEvidencias.DataSource = datos;
        GridEvidencias.DataBind();
    ...
    {
        Estrategias usu = new Estrategias();
        DataRow dato = usu.buscarEvidenciaInfoTecnico(cod);
        if (usu.borrarEvidenciaInfoTecnico(cod))
        {
