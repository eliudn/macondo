...
public partial class sedesincluyenticpei : System.Web.UI.Page
{
    ...
    public void obtenerGET()
    {
        ...
        subtitulo.Text = "Indicador: Sedes educativas que incluyen TIC en los énfasis formativos de sus PEI.";
        descripcion.Text = "";
  ...
  for (int i = 0; i < datos.Rows.Count; i++)
    ...
    [WebMethod(EnableSession = true)]
    public static string sedesincluyenticpei_(string respuesta, string opc)
    {
        ...
        DataTable sedesincluyenticpei = lb.detalleInstitucionesincluyenxRespuesta("", "", "", respuesta1, opc1);


        if (sedesincluyenticpei != null && sedesincluyenticpei.Rows.Count > 0)
        {
            int total = sedesincluyenticpei.Rows.Count;
            ca = "lleno@";
            for (int i = 0; i < sedesincluyenticpei.Rows.Count; i++)
            {
                var f = i + 1;
                ca += "<tr>";
                ca += "<td><b>" + f + ".</b> </td>";
                ca += "<td>" + sedesincluyenticpei.Rows[i]["nombredepartamento"].ToString() + " </td>";
                ca += "<td>" + sedesincluyenticpei.Rows[i]["nombremunicipio"].ToString() + " </td>";
                ca += "<td>" + sedesincluyenticpei.Rows[i]["dane"].ToString() + " - " + sedesincluyenticpei.Rows[i]["nombreins"].ToString() + " </td>";
                ca += "</tr>";
            }
            ca += "@" + total;
        }
    ...
    [WebMethod(EnableSession = true)]
    public static string realizarBusquedasedesincluyenticpei_(string coddepartamento, string codmunicipio, string codinstitucion, string respuesta, string opc)
    {
        string ca = "";
        var lb = new LineaBase();

        DataTable sedesincluyenticpei = lb.detalleInstitucionesincluyenxRespuesta(coddepartamento, codmunicipio, codinstitucion, respuesta, opc);
