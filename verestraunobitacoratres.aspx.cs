...
public partial class verestraunobitacoratres : System.Web.UI.Page
{
    ...
    [WebMethod(EnableSession = true)]
    public static string listarBitacoraTres(string codasesorcoordinador)
    {
        Institucion inst = new Institucion();
        string ca = "";

        DataTable datos = inst.listarBitacoraTresSupervision(codasesorcoordinador);
        if (datos != null && datos.Rows.Count > 0)
        {
            for (int i = 0; i < datos.Rows.Count; i++)
            {
                ...
                ca += "<td style='padding:5px;' ><a class='btn btn-success' onclick=\"$('#table').hide(), $('#form').fadeIn(500),  loadSelectBitacoraTres(" + datos.Rows[i]["codProyecto"].ToString() + "), traerPreguntasTres(" + datos.Rows[i]["codigo"].ToString() + ") \">Ver</a></td>";
                ca += "</tr>";
            }
        }
        else
