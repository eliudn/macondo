...
public partial class estras004Sedes : System.Web.UI.Page
{
    ...
    public static string cargarListadoMemoriaAsesor(string page)
    {
        ...
        DataTable datos = estra.cargarListadoMemoriasS004Sedes(HttpContext.Current.Session["CodAsesorEstraCoordinador"].ToString(), HttpContext.Current.Session["e"].ToString(), HttpContext.Current.Session["m"].ToString(), HttpContext.Current.Session["s"].ToString(), Convert.ToString(offset), Convert.ToString(rowsint), HttpContext.Current.Session["j"].ToString());

        if (datos != null && datos.Rows.Count > 0)
        {
            DataTable datosCount = estra.cargarListadoMemoriasCountSedes(HttpContext.Current.Session["CodAsesorEstraCoordinador"].ToString(), HttpContext.Current.Session["e"].ToString(), HttpContext.Current.Session["m"].ToString(), HttpContext.Current.Session["s"].ToString(), HttpContext.Current.Session["j"].ToString());//Saber la cantidad de registros en la consulta
            ...
            for (int i = 0; i < datos.Rows.Count; i++)
            {
                ...
                ca += "<td align=\"center\"><br /><a class='btn btn-success' onclick=\"$('#listTable').hide(), $('#formTable').fadeIn(500), loadInstrumento(" + datos.Rows[i]["codigo"].ToString() + ")\">Editar</a><br/ ><br /><a href=\"estras004SedesEvidencia.aspx?cod=" + datos.Rows[i]["codigo"].ToString() + "\" class=\"btn btn-primary\">Evidencias</a><br/ ><br /><a href=\"estrag001.aspx?cod=" + datos.Rows[i]["codigo"].ToString() + "&codsede=" + datos.Rows[i]["codsede"].ToString() + "\" class=\"btn btn-success\">Asistencia</a><br /><br /><a onclick=\"eliminar(" + datos.Rows[i]["codigo"].ToString() + ")\" class=\"btn btn-danger\">Eliminar</a><br /><br /></td>";
                ca += "</tr>";
            }
            ca += "@";
            ...
            if ((offset + rowsint) >= Convert.ToInt32(cant)){
                forward = "<li><a href='javascript:void(0);'><img src='Imagenes/flechaOut.png' /></a></li>";
            }
            else
            {
    ...
    public static string deleteestras004(string codigo)
    {
        ...
        {
            DataTable datos = inst.buscarAsistenciasS004Sedes(codigo);
            if (datos != null && datos.Rows.Count > 0)
            {
                inst.eliminarAsistenciaS004Sedes(codigo);

                for (int i = 0; i < datos.Rows.Count; i++)
                {
                    estra.eliminarAsistenciaDetalleDocentesEstra2(datos.Rows[i]["codigo"].ToString());
                }
                estra.borrarEvidenciaEstrategiaS004SedesxCodInstrumento(codigo);
            }
