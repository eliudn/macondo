...
public partial class configrupocambioasesor : System.Web.UI.Page
{
...
//        ScriptManager.RegisterStartupScript(this, typeof(Page), "alerta", script, false);
    ...
    {
        Asesores ase = new Asesores();
        DataTable datos = ase.cargarAsesoresxEstrategia("1");
    ...
    protected void btnCambiarAsesores_Click(object sender, EventArgs e)
    {
        ...
        foreach (GridViewRow row in GridGrupos.Rows)
        {
            ...
            string codasesorcoordinador = HttpUtility.HtmlDecode(row.Cells[3].Text);
            string idbuscar = HttpUtility.HtmlDecode(row.Cells[5].Text);

            DropDownList dd = (row.FindControl("dropAsesores") as DropDownList);
            if (dd.SelectedIndex != 0)
            {
               if(est.editarAsesorEnGrupoInvestigacion(dd.SelectedValue,codproyectosede))
               {
                   val++;

                   DataRow dato = ase.buscarCodUserxIdentificacionAsesor(idbuscar);
                   DataRow dato2 = ase.buscarCodUserxIdentificacionAsesorCoord(dd.SelectedValue);
                   ...
                   if (dato != null)
                   {
                       ...
                       if (dato2 != null)
                       {
                           est.editarCodUsuarioRepositorioS002(dato["cod"].ToString(),dato2["cod"].ToString());
                           est.editarCodUsuarioRepositorioPreestructurados(dato["cod"].ToString(), dato2["cod"].ToString());
                           est.editarCodUsuarioRepositorioEspacioApro(dato["cod"].ToString(), dato2["cod"].ToString());
                           est.editarCodUsuarioG006_Estra1(codasesorcoordinador, dd.SelectedValue);
                           est.editarCodUsuarioRepositorioG006_Estra1(dato["cod"].ToString(), dato2["cod"].ToString());

                           est.editarCodUsuarioG007(codasesorcoordinador, dd.SelectedValue);
                           est.editarCodUsuarioBitacora4punto1(codasesorcoordinador, dd.SelectedValue);
                           est.editarCodUsuarioS002(codasesorcoordinador, dd.SelectedValue);
                       }
                   }
               }
            }
