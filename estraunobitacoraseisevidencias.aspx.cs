...
public partial class estraunobitacoraseisevidencias : System.Web.UI.Page
{
    ...
    {
        lblCodestrabitacora6.Text = HttpUtility.UrlDecode(HttpContext.Current.Request.QueryString["codestrabitacora6"]);
    }
    //Evidencias
    ...
    {
        Response.Redirect("estraunobitacoraseis.aspx?e=" + Session["e"].ToString() + "&m=" + Session["m"].ToString());
    }
    protected void btnActualizarDatosTutoria_Click(object sender, EventArgs e)
    {
        ...
        else if (variables[0] == "exito")
        {
            ...
            if (usu.agregarArchivoRespositorioEstrategia1Asesorbitacora6(lblCodUsuario.Text, variables[2], variables[3], variables[4], variables[5], variables[7], Convert.ToInt32(variables[8]), fun.getFechaAñoHoraActual(), "", lblCodestrabitacora6.Text))
            {
                gvCargarEvidencias();
    ...
    {
        DataTable datos = est.cargarEvidenciasEstrategia1Asesorbitacora6(lblCodestrabitacora6.Text);
    ...
    {
        Estrategias usu = new Estrategias();
        DataRow dato = usu.buscarEvidenciaEstrategia1bitacora6(cod);
        if (usu.borrarEvidenciaEstrategia1bitacora6(cod))
        {
