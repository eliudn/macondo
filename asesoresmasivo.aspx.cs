...
public partial class asesoresmasivo : System.Web.UI.Page
{
    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
        if (FileUpload1.HasFile)
        {
            if (ChecarExtension(FileUpload1.FileName))
            {
                ...
                FileUpload1.SaveAs(Server.MapPath("CSV_Asesores/" + fileNameSave));


                Label1.Text = "<br /><p style='color:#357ebd;font-weight:bold;'>" + FileUpload1.FileName + " cargado exitosamente</p>";

                lblOculto.Text = Server.MapPath("CSV_Asesores/" + fileNameSave);
            }
            else
            {
    ...
    private void recorrerListado()
    {
         ...
         {
             string id = HttpUtility.HtmlDecode(GridView1.Rows[gvr.RowIndex].Cells[0].Text);

             if (id != null && id != "")
              {
                  string nombre = HttpUtility.HtmlDecode(GridView1.Rows[gvr.RowIndex].Cells[1].Text);
                  string apellido = HttpUtility.HtmlDecode(GridView1.Rows[gvr.RowIndex].Cells[2].Text);
                  string telefono = HttpUtility.HtmlDecode(GridView1.Rows[gvr.RowIndex].Cells[3].Text);
                  string email = HttpUtility.HtmlDecode(GridView1.Rows[gvr.RowIndex].Cells[4].Text);
                  string idcoordinador = HttpUtility.HtmlDecode(GridView1.Rows[gvr.RowIndex].Cells[5].Text);
                  string perfil = HttpUtility.HtmlDecode(GridView1.Rows[gvr.RowIndex].Cells[6].Text);

                  DataRow datocoord = ase.buscarAsesorCoordinadorEstrategiaxIdentificacion(idcoordinador);

                 if(datocoord != null)
                 {
                     DataRow dat = ase.agregarAsesor(id, nombre, apellido, telefono, email);
                     if (dat != null)
                     {
                         if (ase.agregarAsesorcoordinador(dat["codigo"].ToString(), datocoord["codestracoordinador"].ToString()))
                         {
                             DataRow usuario = usu.agregarUsuarioPG(id, id, id, nombre, "", apellido, "", telefono, "", email, "On", "");
                             if (usuario != null)
                             {
                                 if (usu.relacionarUsuarioRol(usuario["cod"].ToString(), perfil))
                                     buenos++;
                             }
                             else
                             {
                                 malos++;
                                 ca += "Docente ya tiene Usuario -> " + id + "<br/>";
                             }
                         }
                     }
                     else
                     {
                         malos++;
                         ca += "Error al cargar asesor -> " + id + "<br/>";
                     }
                 }
                 else
                 {
                     malos++;
                     ca += "ID Coordinador no existe en la Base de datos -> " + idcoordinador + "<br/>";
                 }
              }
              else
              {
