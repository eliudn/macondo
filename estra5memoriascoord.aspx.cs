...
public partial class estra5memoriascoord : System.Web.UI.Page
{
    ...
    public void obtenerGET()
    {
        ...
        titulo.Text = "Días de formación de la primera sesión registradas";
    ...
    public static string estra5memorias()
    {
        ...
        DataTable memorias = est.est_s004CoordinadorConMemorias("", "", "", "", "5", HttpContext.Current.Session["s"].ToString());
        if (memorias != null && memorias.Rows.Count > 0)
        {
            int total = memorias.Rows.Count;
            ca = "lleno@";
            for (int i = 0; i < memorias.Rows.Count; i++)
            {
                var f = i + 1;
                ca += "<tr>";
                //ca += "<td><b>" + f + ".</b> </td>";
                ca += "<td>" + memorias.Rows[i]["nombredepartamento"].ToString() + " </td>";
                ca += "<td>" + memorias.Rows[i]["nombremunicipio"].ToString() + " </td>";
                ca += "<td>" + memorias.Rows[i]["lugar"].ToString() + " </td>";
                ca += "<td>" + memorias.Rows[i]["actividad"].ToString() + " </td>";
                ca += "<td>" + fun.convertFechaDia(memorias.Rows[i]["fechaelaboracionini"].ToString()) + " </td>";
                ca += "<td>" + fun.convertFechaDia(memorias.Rows[i]["fechaelaboracion"].ToString()) + " </td>";
                ca += "<td>" + memorias.Rows[i]["horainicio"].ToString() + " </td>";
                ca += "<td>" + memorias.Rows[i]["horafin"].ToString() + " </td>";
    ...
    public static string realizarBusquedaestra4memoriasesion1_(string coddepartamento, string codmuncipio, string codinstitucion, string codsede)
    {
     ...
     {
         Response.Redirect("estra5sesiones.aspx?s=" + Session["s"].ToString());
         //if (Session["m"].ToString() != null)
         //{
         //    if (Session["m"].ToString() == "1")
         //    {
