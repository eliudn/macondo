...
public partial class estras002evidencias : System.Web.UI.Page
{
    ...
    {
        lblCodestras002.Text = HttpUtility.UrlDecode(HttpContext.Current.Request.QueryString["codestras002"]);
    ...
    {
        if (Session["codrol"].ToString() == "10" || Session["codrol"].ToString() == "20" || Session["codrol"].ToString() == "18")
        {
            Response.Redirect("estras002reporte.aspx?e=" + Session["e"].ToString() + "&m=" + Session["m"].ToString() + "&s=" + Session["s"].ToString() + "&a=" + Session["a"].ToString());
        }
        else if (Session["codrol"].ToString() == "11")
        {
            Response.Redirect("estras002.aspx?e=" + Session["e"].ToString() + "&m=" + Session["m"].ToString() + "&s=" + Session["s"].ToString());
        }
    ...
    protected void btnActualizarDatosTutoria_Click(object sender, EventArgs e)
    {
        ...
        else if (variables[0] == "exito")
        {
            ...
            if (usu.agregarArchivoRespositorioEstrategia1AsesorS002(lblCodUsuario.Text, variables[2], variables[3], variables[4], variables[5], variables[7], Convert.ToInt32(variables[8]), fun.getFechaAñoHoraActual(),  rbtActividades.SelectedValue, lblCodestras002.Text))
            {
                gvCargarEvidencias();
    ...
    {
        DataTable datos = est.cargarEvidenciasEstrategia1AsesorS002(lblCodestras002.Text);
    ...
    {
        Estrategias usu = new Estrategias();
        DataRow dato = usu.buscarEvidenciaEstrategia1S002(cod);
        if (usu.borrarEvidenciaEstrategia1S002(cod))
        {
