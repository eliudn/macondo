...
public partial class confiespaciodeapropiacionaprodocevi : System.Web.UI.Page
{
    ...
    {
        Response.Redirect("confiespaciodeapropiacionaprodoc.aspx?cod=" + Session["CodApro"].ToString());
    }
    protected void btnActualizarDatosTutoria_Click(object sender, EventArgs e)
    {
        ...
        else if (variables[0] == "exito")
        {
            ...
            if (est.agregarArchivoRespositorioEspacioAproDocente(lblCodUsuario.Text, variables[2], variables[3], variables[4], variables[5], variables[7], Convert.ToInt32(variables[8]), fun.getFechaAñoHoraActual(), lblCodEspacio.Text, rbtActividades.SelectedValue))
            {
                //gvCargarEvidencias();
    ...
    {
        DataTable datos = est.cargarEvidenciasEspacioAproDocente(lblCodEspacio.Text, rbtActividadesBuscar.SelectedValue, lblCodUsuario.Text);
    ...
    private void borrarImagen(string cod)
    {
        ...
        DataRow dato = usu.buscarEvidenciaEspacioAproDocente(cod);
        if (usu.borrarEvidenciaEspacioAproDocente(cod))
        {
