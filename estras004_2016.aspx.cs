...
public partial class estras004_2016 : System.Web.UI.Page
{
    ...
    public static string insertestras004(string redtematica, string nombresesion, string temasesion, string informacionadicional, string fechaelaboracion, string nombrerelator, string horasesion, string aspectosdesarrollados, string conclusiones, string bibliografia)
    {
        ...
        Institucion inst = new Institucion();
        string ca = "";
        estras004_2016 estra = new estras004_2016();
        Funciones fun = new Funciones();

        DataRow insert = inst.insertestras004_2016(redtematica, nombresesion, temasesion, informacionadicional, fun.convertFechaAño(fechaelaboracion), nombrerelator, horasesion, HttpContext.Current.Session["CodAsesorEstraCoordinador"].ToString(), aspectosdesarrollados, conclusiones, bibliografia, HttpContext.Current.Session["e"].ToString(), HttpContext.Current.Session["m"].ToString(), HttpContext.Current.Session["s"].ToString());
    ...
    public static string loadestras004(string codigo)
    {
        ...
        DataRow datosinstrumento = inst.proloadestras004_2016(codigo, HttpContext.Current.Session["e"].ToString(), HttpContext.Current.Session["m"].ToString(), HttpContext.Current.Session["s"].ToString());
    ...
    public static string updateestras004(string codigoestrategia, string redtematica, string nombresesion, string temasesion, string informacionadicional, string fechaelaboracion, string nombrerelator, string horasesion, string aspectosdesarrollados, string conclusiones, string bibliografia)
    {
        ...
        long update = inst.procupdateestras004_2016(codigoestrategia, redtematica, nombresesion, temasesion, informacionadicional, fechaelaboracion, nombrerelator, horasesion, aspectosdesarrollados, conclusiones, bibliografia);
        if (update != -1)
        {
            ca = "true@";
        }
    ...
    public static string deleteestras004(string codigo)
    {
        ...
        if (inst.eliminarMemoriaS004_2016(codigo))
        {
    ...
    public static string cargarRedtematica(string codsede)
    {
        ...
        if(HttpContext.Current.Session["codrol"] != null)
        {
            ...
            {
                DataTable datos = inst.proccargarRedtematica_2016(codsede, HttpContext.Current.Session["CodAsesorEstraCoordinador"].ToString());
    ...
    {
        DataTable datos = est.cargarEvidenciasEstrategia4AsesorRedTematica_2016(lblMomento.Text, lblSesion.Text, "4", HttpContext.Current.Session["codredtematicasede"].ToString(), lblCodUsuario.Text);
    ...
    public static string cargarListadoMemoriaAsesor(string page)
    {
        ...
        DataTable datos = estra.cargarListadoMemoriasS004_2016(HttpContext.Current.Session["CodAsesorEstraCoordinador"].ToString(), HttpContext.Current.Session["e"].ToString(), HttpContext.Current.Session["m"].ToString(), HttpContext.Current.Session["s"].ToString(), Convert.ToString(offset), Convert.ToString(rowsint));

        if (datos != null && datos.Rows.Count > 0)
        {
            DataTable datosCount = estra.cargarListadoMemoriasCount_2016(HttpContext.Current.Session["CodAsesorEstraCoordinador"].ToString(), HttpContext.Current.Session["e"].ToString(), HttpContext.Current.Session["m"].ToString(), HttpContext.Current.Session["s"].ToString());//Saber la cantidad de registros en la consulta
            ...
            for (int i = 0; i < datos.Rows.Count; i++)
            {
                ...
                ca += "<td align=\"center\"><br/ ><a class='btn btn-success' onclick=\"$('#listTable').hide(), $('#formTable').fadeIn(500), loadInstrumento(" + datos.Rows[i]["codigo"].ToString() + ")\">Editar</a><br/ ><br /><a onclick=\"evidencias(" + datos.Rows[i]["codigo"].ToString() + ")\" class=\"btn btn-primary\">Evidencias</a><br/ ><br /><br/ ><br /><a onclick=\"eliminar(" + datos.Rows[i]["codigo"].ToString() + ")\" class=\"btn btn-danger\">Eliminar</a><br/ ><br /></td>";
                ca += "</tr>";
                //<a href=\"estrag001_estrategia4_2016.aspx?codinstrumentos004=" + datos.Rows[i]["codigo"].ToString() + "&codredtematicasede=" + datos.Rows[i]["codredtematicasede"].ToString() + " \" class=\"btn btn-success\">Asistencias</a>
            }
            ca += "@";
    ...
    public static string loadEvidencias(string codigo)
    {
        string ca = "";
        Estrategias estra = new Estrategias();
        DataTable datos = estra.cargarEvidenciasEstrategia4AsesorRedTematica_2016(HttpContext.Current.Session["m"].ToString(), HttpContext.Current.Session["s"].ToString(), "4", codigo, HttpContext.Current.Session["codusuario"].ToString());
