...
public partial class estrag007 : System.Web.UI.Page
{
    ...
    public static string updateRubros(string codestrategia, string valInsumo, string valPapeleria, string valTransporte, string valMaterial, string valRefrigerio, string valOtros, string valInsumo2do, string valPapeleria2do, string valTransporte2do, string valMaterial2do, string valRefrigerio2do, string valOtros2do, string valInsumo3ro, string valPapeleria3ro, string valTransporte3ro, string valMaterial3ro, string valRefrigerio3ro, string valOtros3ro)
    {
        ...
        //if (row != null)
        //{
        if (estra.actualizarRubros(codestrategia, valInsumo, valPapeleria, valTransporte, valMaterial, valRefrigerio, valOtros, valInsumo2do, valPapeleria2do, valTransporte2do, valMaterial2do, valRefrigerio2do, valOtros2do, valInsumo3ro, valPapeleria3ro, valTransporte3ro, valMaterial3ro, valRefrigerio3ro, valOtros3ro))
        {
    ...
    [WebMethod(EnableSession = true)]
    public static string guardarRubros(string codGrupoInvestigacion, string valInsumo, string valPapeleria, string valTransporte, string valMaterial, string valRefrigerio, string valOtros, string valInsumo2do, string valPapeleria2do, string valTransporte2do, string valMaterial2do, string valRefrigerio2do, string valOtros2do, string valInsumo3ro, string valPapeleria3ro, string valTransporte3ro, string valMaterial3ro, string valRefrigerio3ro, string valOtros3ro)
    {
        string ca = "";
        Estrategias estra = new Estrategias();

       
        if (estra.agregarRubros(HttpContext.Current.Session["CodEstraAsesorCoordinador"].ToString(), codGrupoInvestigacion, valInsumo, valPapeleria, valTransporte, valMaterial, valRefrigerio, valOtros, HttpContext.Current.Session["e"].ToString(), HttpContext.Current.Session["m"].ToString(), HttpContext.Current.Session["s"].ToString(), valInsumo2do, valPapeleria2do, valTransporte2do, valMaterial2do, valRefrigerio2do, valOtros2do, valInsumo3ro, valPapeleria3ro, valTransporte3ro, valMaterial3ro, valRefrigerio3ro, valOtros3ro))
        {
    ...
    public static string searchRubros(string codigo)
    {
        ...
        if(dato != null){
            ...
            ca += dato["insumo2do"].ToString() + "-";
            ca += dato["papeleria2do"].ToString() + "-";
            ca += dato["transporte2do"].ToString() + "-";
            ca += dato["material2do"].ToString() + "-";
            ca += dato["refrigerios2do"].ToString() + "-";
            ca += dato["otro2do"].ToString() + "-";

            ca += dato["insumo3ro"].ToString() + "-";
            ca += dato["papeleria3ro"].ToString() + "-";
            ca += dato["transporte3ro"].ToString() + "-";
            ca += dato["material3ro"].ToString() + "-";
            ca += dato["refrigerios3ro"].ToString() + "-";
            ca += dato["otro3ro"].ToString();
    ...
    [WebMethod(EnableSession = true)]
    public static string listadobitacora4()
    {
        string ca = "";

        Estrategias estra = new Estrategias();

        DataTable datos = estra.listarBitacoraCuatroSupervision(HttpContext.Current.Session["CodEstraAsesorCoordinador"].ToString());
        if (datos != null && datos.Rows.Count > 0)
        {
            for (int i = 0; i < datos.Rows.Count; i++)
            {
                ...
                ca += "<td style='padding:5px;text-align:center' ><a class='btn btn-success' onclick=\"$('#table').hide(), $('#form').fadeIn(500),  loadSelectBitacoraCuatro(" + datos.Rows[i]["codigo"].ToString() + "), loadRubros(" + datos.Rows[i]["codigo"].ToString() + ") \">Ver</a><br /><br /><a href='estrag007Evidencias.aspx?codestrag007=" + datos.Rows[i]["codigo"].ToString() + "' class='btn btn-primary'>Evidencias</a><br/><br/><a href='estraunobitacoraseisevidenciapresupuesto.aspx?codestrag007=" + datos.Rows[i]["codigo"].ToString() + "' class='btn btn-primary'>Historial Bitacora</a><br/><br/><a class='btn btn-danger' onclick=\"deleteBitacoraCuatro(" + datos.Rows[i]["codigo"].ToString() + ")\">Eliminar</a></td>";
                ca += "</tr>";
            }
        }
        else
    ...
    public static string deletebitacora(string codigo)
    {
        ...
        if (est.eliminarBitacoraCuatro(codigo))
        {
            est.borrarEvidenciaEstrategia1g007xCodestrategia(codigo);
            ca = "delete@";
        }
        else
        {
