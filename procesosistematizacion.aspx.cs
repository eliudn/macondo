...
public partial class procesosistematizacion : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            if (Session["codrol"] != null)
            {
                ...
                if(lblEstrategia.Text != "" && lblMomento.Text != "")
                {
                    buscarUsuario();
                    gvCargarEvidencias();

                    if (lblEstrategia.Text == "1")
                    {
                        ca = "<a href='Proceso_Sistematizacion/ESTRATEGA_1_INSTRUCTIVO_UNIMAG.docx' target='_blank' class='btn btn-danger'>Instructivo</a>";
                    }
                    else if (lblEstrategia.Text == "2")
                    {
                        ca = "<a href='Proceso_Sistematizacion/ESTRATEGIA_2_INSTRUCTIVO_CUC.docx' target='_blank' class='btn btn-danger'>Instructivo</a>";
                    }
                    else if (lblEstrategia.Text == "3")
                    {
                        ca = "<a href='Proceso_Sistematizacion/ESTRATEGIA_3_INSTRUCTIVO_FUNTICS.docx' target='_blank' class='btn btn-danger'>Instructivo</a>";
                    }
                    else if (lblEstrategia.Text == "4")
                    {
                        ca = "<a href='Proceso_Sistematizacion/ESTRATEGIA_4_INSTRUCTIVO_FUNTICS.docx' target='_blank' class='btn btn-danger'>Instructivo</a>";
                    }
                    else if (lblEstrategia.Text == "5")
                    {
                        ca = "<a href='Proceso_Sistematizacion/ESTRATEGIA_5_INSTRUCTIVO_UNIMAG.docx' target='_blank' class='btn btn-danger'>Instructivo</a>";
                    }
                    lblInstructivo.Text = ca;
                }
    ...
    protected void btnActualizarDatosTutoria_Click(object sender, EventArgs e)
    {
        ...
        else if (variables[0] == "exito")
        {
            ...
            if (usu.agregarArchivoRespositorioProcesoSistematizacion(lblCodUsuario.Text, variables[2], variables[3], variables[4], variables[5], variables[7], Convert.ToInt32(variables[8]), fun.getFechaAñoHoraActual(), lblEstrategia.Text, lblMomento.Text, rbtActividades.SelectedValue))
                {
                    gvCargarEvidencias();
    ...
    private string[] subirArchivo(FileUpload trepador)
    {
        ...
        if (HttpContext.Current.Request.ApplicationPath.Remove(0, 1) == "")
            pathsinmap = "~/Proceso_Sistematizacion/";
        else
            pathsinmap = "~/Proceso_Sistematizacion/" + HttpContext.Current.Request.ApplicationPath.Remove(0, 1) + "/";
    ...
    {
        DataTable datos = est.cargarEvidenciasProcesoSistematizacion(lblCodUsuario.Text, lblEstrategia.Text, lblMomento.Text);
    ...
    {
        Estrategias usu = new Estrategias();
        DataRow dato = usu.buscarEvidenciaProcesoSistematizacion(cod);
        if (usu.borrarEvidenciaProcesoSistematizacion(cod))
        {
