...
public partial class modificacionCorriculo : System.Web.UI.Page
{
    ...
    public void obtenerGET()
    {
        ...
        subtitulo.Text = "Indicador: •	Modificaciones a los currículo dirigidas a introducir la IEP apoyada en TIC";
        descripcion.Text = "";
  ...
  for (int i = 0; i < datos.Rows.Count; i++)
    ...
    [WebMethod(EnableSession = true)]
    public static string modificacionCorriculo_(string respuesta)
    {
        ...
        DataTable plataformaspedagogicassedes = lb.modcorriculo("", "", "", "",  respuesta);
        ...
        if (plataformaspedagogicassedes != null && plataformaspedagogicassedes.Rows.Count > 0)
        {
            ...
            for (int i = 0; i < plataformaspedagogicassedes.Rows.Count; i++)
            {
                ...
                ca += "<td>" + plataformaspedagogicassedes.Rows[i]["count"].ToString() + " </td>";
                ca += "<td>" + plataformaspedagogicassedes.Rows[i]["respuesta"].ToString() + " </td>";
                ca += "</tr>";
            }
            ca += "@" + total;
        }
    ...
    [WebMethod(EnableSession = true)]
    public static string buscarModificacionCorriculo_(string coddepartamento, string codmunicipio, string codinstitucion, string codsede, string respuesta, string opc)
    {
        string ca = "";
        var lb = new LineaBase();

        DataTable plataformaspedagogicassedes = lb.modcorriculo(coddepartamento, codmunicipio, codinstitucion, codsede, respuesta);
