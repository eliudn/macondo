...
public partial class ginvestigacionlistado_evi : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        PanelEdicion.Attributes.Add("style", "display:block");
        //PanelVer.Attributes.Add("style", "display:block");
        if (!IsPostBack)
        {
                buscarUsuario();
                obtenerGet();
                ...
                if (lblCod.Text != "")
                {
                    gvCargarGridGruposInvestigacionEvidencias(lblCod.Text);
                }
                else
                {
                    mostrarmensaje("error", "Error al ver evidencias, no se recibieron los parámetros");
                }
        }
    }
    ...
    {
        lblCod.Text = HttpUtility.UrlDecode(HttpContext.Current.Request.QueryString["cod"]);
    }
    ...
    protected void btnActualizarDatosTutoria_Click(object sender, EventArgs e)
    {
        ...
        else if (variables[0] == "exito")
        {
            ...
            if (ins.agregarArchivoRespositorioEstrategia(lblCodUsuario.Text, variables[2], variables[3], variables[4], variables[5], variables[7], Convert.ToInt32(variables[8]), fun.getFechaAñoHoraActual(), lblCod.Text))
                {
                    gvCargarGridGruposInvestigacionEvidencias(lblCod.Text);
    ...
    private void gvCargarGridGruposInvestigacionEvidencias(string codgrupoinvestigacion)
    {
        DataTable datos = ins.cargarEvidenciasGruposInvestigacion(codgrupoinvestigacion);
    ...
    protected void imgSubir_Click(object sender, ImageClickEventArgs e)
    {
        ...
        //string cod = GridGruposInvestigacion.DataKeys[gvrow.RowIndex].Value.ToString(); //Obtener del DataKey de la Row  

        //lblCodGrupoInvestigacion.Text = cod;

        //this.PanelEdicion_ModalPopupExtender.Show();
    ...
    protected void imgVer_Click(object sender, ImageClickEventArgs e)
    {
        ...
        //this.PanelVer_ModalPopupExtender.Show();

        //gvCargarGridGruposInvestigacionEvidencias(cod);
    }
    ...
    private void borrarImagen(string cod)
    {
        ...
        DataRow dato = usu.buscarEvidenciaGrupoInvestigacion(cod);
        if (usu.borrarEvidenciaGrupoInvestigacion(cod))
        {
