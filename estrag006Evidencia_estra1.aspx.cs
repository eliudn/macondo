...
public partial class estrag006Evidencia_estra1 : System.Web.UI.Page
{
    ...
    protected void btnActualizarDatosTutoria_Click(object sender, EventArgs e)
    {
        ...
        else if (variables[0] == "exito")
        {
            ...
            if (usu.agregarArchivoRespositorioEntregaMaterialEstra1(lblCodUsuario.Text, variables[2], variables[3], variables[4], variables[5], variables[7], Convert.ToInt32(variables[8]), fun.getFechaAñoHoraActual(), lblCodInstrumento.Text))
                {
                    gvCargarEvidencias();
    ...
    {
        DataTable datos = est.cargarEvidenciasEntregaMaterialEstra1(lblCodInstrumento.Text, lblCodUsuario.Text);
    ...
    {
        Estrategias usu = new Estrategias();
        DataRow dato = usu.buscarEvidenciaEntregaMaterialEstra1(cod);
        if (usu.borrarEvidenciaEntregaMaterialEstra1(cod))
        {
    ...
    {
        if (Session["codrol"].ToString() == "11")//Asesor unimag
        {
            Response.Redirect("estrag006_estrategia1.aspx");
        }
        else
        {
            Response.Redirect("estrag006Reporte_estra1.aspx");
        }
