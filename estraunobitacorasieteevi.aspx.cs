...
public partial class estraunobitacoradosevi : System.Web.UI.Page
{
    ...
    {
        if (Session["codrol"].ToString() == "10" || Session["codrol"].ToString() == "18" || Session["codrol"].ToString() == "20")
            Response.Redirect("estraunobitacorasiete.aspx?e=" + Session["e"].ToString() + "&m=" + Session["m"].ToString() + "&s=" + Session["s"].ToString());
        else
            Response.Redirect("estraunobitacorasiete.aspx?e=" + Session["e"].ToString() + "&m=" + Session["m"].ToString() + "&s=" + Session["s"].ToString());
    }
    protected void btnActualizarDatosTutoria_Click(object sender, EventArgs e)
    {
        ...
        else if (variables[0] == "exito")
        {
            ...
            if (usu.agregarArchivoRespositorioEstrategia1Asesorbitacora5(lblCodUsuario.Text, variables[2], variables[3], variables[4], variables[5], variables[7], Convert.ToInt32(variables[8]), fun.getFechaAñoHoraActual(), "", lblCodestrabitacora2.Text))
            {
                gvCargarEvidencias();
