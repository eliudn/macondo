...
public partial class estraunoevidenciasactividad : System.Web.UI.Page
{
    ...
    public void obtenerGET()
    {
       ...
       lblCodUsuario.Text = HttpUtility.UrlDecode(HttpContext.Current.Request.QueryString["cod"]);
        ...
        {
            actividadactual.Text = "<h2>1. Plan operativo aprobado por interventoría</h2>";
        }
        else if (Session["a"].ToString() == "2")
        {
            actividadactual.Text = "<h2>2. Ruta metodológica de investigación e innovación del Proyecto Ciclón</h2>";
        }
        else if (Session["a"].ToString() == "3")
        {
            actividadactual.Text = "<h2>3. Jornadas de formación dirigidas a los estudiantes</h2>";
        }
        else if (Session["a"].ToString() == "4")
        {
            actividadactual.Text = "<h2>4. Lineamientos pedagógicos y logisticos de las ferias institucionales</h2>";
        }
        else if (Session["a"].ToString() == "5")
        {
            actividadactual.Text = "<h2>5. Cronograma de trabajo conjunto para organizar la llegada a las instituciones educativas</h2>";
        }
        else if (Session["a"].ToString() == "6")
        {
            actividadactual.Text = "<h2>6. Actas de aprobación comité técnicos</h2>";
        }
    }
    ...
    protected void btnActualizarDatosTutoria_Click(object sender, EventArgs e)
    {
        ...
        else if (variables[0] == "exito")
        {
            ...
            {
                if (usu.agregarArchivoRespositorioEstrategia(lblCodUsuario.Text, variables[2], variables[3], variables[4], variables[5], variables[7], Convert.ToInt32(variables[8]), fun.getFechaAñoHoraActual(), lblMomento.Text, lblSesion.Text, HttpContext.Current.Session["a"].ToString(), "1"))
                {
                    gvCargarEvidencias();
    ...
    private void gvCargarEvidencias()
    {
        ...
        {
            DataTable datos2 = est.cargarEvidenciasEstrategiaConActividad(lblMomento.Text, lblSesion.Text, "1", HttpContext.Current.Session["a"].ToString(), lblCodUsuario.Text);//(string momento, string sesion, string estrategia, string actividad, string codusuario)
    ...
    {
        if (Session["codrol"].ToString() == "9")
        {
            Response.Redirect("listestraunoevidenciassupervisor.aspx?m=" + lblMomento.Text + "&s=" + lblSesion.Text + "&codac=" + lblCodUsuario.Text);
        }else{
            Response.Redirect("listestraunoevidencias.aspx?m=" + lblMomento.Text+"&s="+ lblSesion.Text);
        }
    }
