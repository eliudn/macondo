...
public partial class estraunos008 : System.Web.UI.Page
{
    ...
    [WebMethod(EnableSession = true)]
    public static string listarBitacoraSeis()
    {
        Institucion inst = new Institucion();
        string ca = "";

        DataTable datos = inst.listarS008(HttpContext.Current.Session["CodAsesorEstraCoordinador"].ToString(), HttpContext.Current.Session["e"].ToString(), HttpContext.Current.Session["m"].ToString());
        if (datos != null && datos.Rows.Count > 0)
        {
            for (int i = 0; i < datos.Rows.Count; i++)
            {
                ...
                ca += "<td style='padding:5px;' ><a class='btn btn-success' onclick=\"$('#table').hide(), $('#form').fadeIn(500),  loadSelectBitacoraSeis(" + datos.Rows[i]["codproyectosede"].ToString() + "), loadInstrumentosSeis(" + datos.Rows[i]["codigo"].ToString() + ") \">Editar</a><br/><br/><a class='btn btn-danger' onclick='eliminar(" + datos.Rows[i]["codigo"].ToString() + ")'>Eliminar</a><br/></td>";
                ca += "</tr>";
            }
        }
        else
