...
public partial class estraunojornadaformacionevi : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            //decimal valor = 30.20m;
            //decimal total = Decimal.Round(valor, 2);

            //decimal valor2 = 30.20m;
            //decimal total2 = Decimal.Round(valor2, 2);

            //decimal vtotal = total + total2;
    ...
    {
        Response.Redirect("estraunojornadaformacion.aspx?m=" + Session["m"].ToString() + "&j=" + Session["j"].ToString());
    }
    protected void btnActualizarDatosTutoria_Click(object sender, EventArgs e)
    {
        ...
        else if (variables[0] == "exito")
        {
            ...
            if (usu.agregarArchivoRespositorioEstrategia1JornadaFormacion(lblCodUsuario.Text, variables[2], variables[3], variables[4], variables[5], variables[7], Convert.ToInt32(variables[8]), fun.getFechaAñoHoraActual(), rbtActividades.SelectedValue, lblCodigo.Text))
            {
                gvCargarEvidencias();
    ...
    {
        DataTable datos = est.cargarEvidenciasEstrategia1CoordJornadaFormacion(lblCodigo.Text);
    ...
    {
        Estrategias usu = new Estrategias();
        DataRow dato = usu.buscarEvidenciaEstrategia1JornadaFormacion(cod);
        if (usu.borrarEvidenciaEstrategia1JornadaFormacion(cod))
        {
