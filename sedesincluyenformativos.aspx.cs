...
public partial class sedesincluyenformativos : System.Web.UI.Page
{
    ...
    public void obtenerGET()
    {
        ...
        subtitulo.Text = "Indicador: Sedes educativas que incluyen ciencia, innovación, investigación y TIC en los énfasis formativos de sus PEI.";
        descripcion.Text = "";
  ...
  for (int i = 0; i < datos.Rows.Count; i++)
    ...
    [WebMethod(EnableSession = true)]
    public static string sedesincluyenformativos_(string respuesta)
    {
        ...
        DataTable sedesincluyenformativos = lb.detalleSedesIncluyenFormativos("", "", "", respuesta1);


        if (sedesincluyenformativos != null && sedesincluyenformativos.Rows.Count > 0)
        {
            int total = sedesincluyenformativos.Rows.Count;
            ca = "lleno@";
            for (int i = 0; i < sedesincluyenformativos.Rows.Count; i++)
            {
                var f = i + 1;
                ca += "<tr>";
                ca += "<td><b>" + f + ".</b> </td>";
                ca += "<td>" + sedesincluyenformativos.Rows[i]["nombredepartamento"].ToString() + " </td>";
                ca += "<td>" + sedesincluyenformativos.Rows[i]["nombremunicipio"].ToString() + " </td>";
                ca += "<td>" + sedesincluyenformativos.Rows[i]["dane"].ToString() + " - " + sedesincluyenformativos.Rows[i]["nombreins"].ToString() + " </td>";
                ca += "</tr>";
            }
            ca += "@" + total;
        }
    ...
    [WebMethod(EnableSession = true)]
    public static string realizarBusquedasedesincluyenformativos_(string coddepartamento, string codmunicipio, string codinstitucion, string respuesta)
    {
        string ca = "";
        var lb = new LineaBase();

        DataTable sedesincluyenformativos = lb.detalleSedesIncluyenFormativos(coddepartamento, codmunicipio, codinstitucion, respuesta);
