...
public partial class estraunoparticipantesferias : System.Web.UI.Page
{
    ...
    [WebMethod(EnableSession = true)]
    public static string listarParticipantesFerias()
    {
        Institucion inst = new Institucion();
        Funciones fun = new Funciones();
        string ca = "";

        DataTable datos = inst.listarParticipantesFerias(HttpContext.Current.Session["CodAsesorEstraCoordinador"].ToString());
        if (datos != null && datos.Rows.Count > 0)
        {
            for (int i = 0; i < datos.Rows.Count; i++)
            {
                ...
                ca += "<td style='padding:5px;' ><a class='btn btn-success' onclick=\"$('#table').hide(), $('#form').fadeIn(500),  loadSelectParticipantesFerias(" + datos.Rows[i]["codsede"].ToString() + "), loadInstrumentosParticipantesFerias(" + datos.Rows[i]["codigo"].ToString() + ") \">Editar</a><br/><br/><a class='btn btn-danger' onclick='eliminar(" + datos.Rows[i]["codigo"].ToString() + ")'>Eliminar</a><br/></td>";
