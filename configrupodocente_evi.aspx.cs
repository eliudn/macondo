...
public partial class configrupodocente_evi : System.Web.UI.Page
{
    ...
    protected void btnActualizarDatosTutoria_Click(object sender, EventArgs e)
    {
        ...
        else if (variables[0] == "exito")
        {
            ...
            if (usu.agregarArchivoGrupoDocentesEstra2(lblCodUsuario.Text, variables[2], variables[3], variables[4], variables[5], variables[7], Convert.ToInt32(variables[8]), fun.getFechaAñoHoraActual(), lblCodInstrumento.Text, rbtActividades.SelectedValue))
                {
                    gvCargarEvidencias();
    ...
    {
        DataTable datos = est.cargarEvidenciasGrupoDocentes(lblCodInstrumento.Text, lblCodUsuario.Text);
    ...
    {
        Estrategias usu = new Estrategias();
        DataRow dato = usu.buscarEvidenciaGrupoDocentes(cod);
        if (usu.borrarEvidenciaGrupoDocentes(cod))
        {
