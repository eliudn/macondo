 <%@ Page Title="Indicadores Departamentales " Language="C#" MasterPageFile="~/General.master" AutoEventWireup="true" CodeFile="indicadoresgeneral.aspx.cs" Inherits="indicadoresgeneral" MaintainScrollPositionOnPostback="true" %>
    ...
    <style type="text/css">
     ...
     <script type="text/javascript">
        $(document).ready(function () {
            ...
            $.ajax({
                type: 'POST',
                url: 'indicadoresgeneral.aspx/cargarDepartamentoMagdalena',
                ...
                success: function (response) {
                    ...
                    //$.ajax({
                    //    type: 'POST',
                    //    url: 'indicadoresgeneral.aspx/cargarMunicipios',
                    //    data: jsondata,
                    //    contentType: 'application/json; charset=utf-8',
                    //    dataType: 'JSON',
                    //    success: function (response) {
                    ...
                    $("#title-report").html("Indicadores objetivo general del Departamento del Magdalena");
        ...
        function floadDetalles() {
            //$.ajax({
            //    type: 'POST',
            //    url: 'indicadoresgeneral.aspx/loadreport',
            //    contentType: 'application/json; charset=utf-8',
            //    dataType: 'JSON',
            //    success: function (response) {
            //        //var resp = response.d.split("@");
        ...
        function filtro() {
            ...
            $.ajax({
                type: 'POST',
                url: 'indicadoresgeneral.aspx/realizarBusquedaloadreport',
                data: jsondata,
                contentType: "application/json; charset=utf-8",
                dataType: "json",
                success: function (json) {
                    ...
                    if (resp[0] == "lleno") {
                        ...
                        $("#infodetmunicipios tbody").html(resp[4]);
                        $("#infodetsedes tbody").html(resp[5]);
                        //$("#infodetdocentes tbody").html(resp[9]);

                        var muni = resp[2].split(",");
                        var totalmuni = parseFloat(muni[0] + "." + muni[1]);

                        var sede = resp[3].split(",");
                         totalsede = parseFloat(sede[0] + "." + sede[1]);

                        var maes = resp[4].split(",");
                        var totalmaes = parseFloat(maes[0] + "." + maes[1]);

                        var estu = resp[5].split(",");
                        var totalestu = parseFloat(estu[0] + "." + estu[1]);

                        var publi = resp[6].split(",");
                        var totalpubli = parseFloat(publi[0] + "." + publi[1]);

                        
                        var totall = (totalmuni + totalsede + totalmaes + totalestu + totalpubli);
                        //var totall = (totalmaes + totalestu);
...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="Server">
    ...
    <br />
     ...
     <center>
            <a href="indicadoresgeneral2.aspx" class="btn btn-success" >Ver indicadores</a>
             <%--<a href="indicadoresestudiantes.aspx" class="btn btn-success" >Ver indicadores Estudiantes</a>
             <a href="indicadoresmaestros.aspx" class="btn btn-success" >Ver indicadores Docentes</a>--%>
...
<fieldset>
 ...
 <!--
        ...
        <table width="100%" class="border" id="tableList">
         <thead>
		    <tr>
		        ...
		        <th style="width:45%">Indicadores del Objetivo General</th>
                <th>Ejecución</th>
    ...
    <div id="detmunicipios" style="display:none">
        <fieldset>
            <legend>Municipios impactados</legend>
             <table width="100%" class="border" id="infodetmunicipios">
                 <thead>
		            <tr>
                        <th style="width: 20px;">No.</th>
        ...
        <a href='javascript:void(0)' class="btn btn-primary" onclick="$('#detmunicipios').hide();$('#detsede').hide();$('#detalle').show()">Regresar</a>
    </div>

     <div id="detsede" style="display:none">
        <fieldset>
            <legend>Sedes educativas públicas beneficiadas</legend>
             <table width="100%" class="border" id="infodetsedes">
                 <thead>
		            <tr>
                        <th style="width: 20px;">No.</th>
		                <th style="width:45%">Municipio</th>
                        <th>Institución</th>
		                <th class="noExl">Sede</th>
		            </tr>
                </thead>
                 <tbody>
    ...
    <%--<div id="detdocente" style="display:none">
        <fieldset>
            <legend>Maestros(as) de las intituciones educativas participantes</legend>
             <table width="100%" class="border" id="infodetdocentes">
                 <thead>
		            <tr>
                        <th style="width: 20px;">No.</th>
		                <th style="width:45%">Municipio</th>
