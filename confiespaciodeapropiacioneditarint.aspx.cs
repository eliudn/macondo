        ...
        if (!IsPostBack)
        {
            ...
            if (Session["codigo"] != null)
            {
                ddMunicipios(dropMunicipiosSedes, "20");
                    DataRow dat = est.buscarFeriaInternacionalxcodigo(Session["codigo"].ToString());
            ...
            else
            {
                ...
                lblError.Text = "Error al crear o ver Feria Internacional";
            }
...
//        ScriptManager.RegisterStartupScript(this, typeof(Page), "alerta", script, false);
    ...
    protected void btnAgregarEstudiantes_Click(object sender, EventArgs e)
    {
        ...
        bool feriam = est.actualizarFeriaInternacional(dropDepartamento.SelectedValue, nombreferiamunicipal.Text, numeroasistentes.Text, numerogrupos.Text, numerodocentes.Text, lugar.Text, fechaelaboracion.Text, horaferia.SelectedValue, horaferiafinal.SelectedValue, fechaelaboracioncierre.Text, horaferiacierre.SelectedValue, horaferiafinalcierre.SelectedValue, Session["codigo"].ToString());

        string script = @"<script type='text/javascript'>$('#CrearRed').hide(),$('#VerRed').fadeIn(500),alert('Feria Internacional actualizada exitosamente.');
                     </script>";
