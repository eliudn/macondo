...
public partial class estraunoespaciosapro : System.Web.UI.Page
{
    ...
    [WebMethod(EnableSession = true)]
    public static string listardocentesmatriculados(string codespacioapro, string codsede, string codanio)
    {
        string ca = "";
        Docentes estu = new Docentes();
        ...
        if (lista2 != null && lista2.Rows.Count > 0)
        {
            ...
            for (int j = 0; j < lista2.Rows.Count; j++)
            {
                ...
                DataRow lista = estu.buscarDocentesEspacioApropiacion(codespacioapro, lista2.Rows[j]["codgradodocente"].ToString());
    ...
    [WebMethod(EnableSession = true)]
    public static string matricularDocentes(string codespacioapro, string codgradodocente)
    {
        string ca = "";
        Docentes estu = new Docentes();
        ...
        {
            bool response = estu.matriculaDocenteEspacioApro(codespacioapro, codgdocente[i].ToString());
            if (response)
            {
    ...
    public static string deletematriculaDocente(string codespacioapro)
    {
       ...
       if(estu.deleteDocentesEspacioApropiacion(codespacioapro)){
    ...
    [WebMethod(EnableSession = true)]
    public static string insertInstrumento(string codproyecto, string fecharealizacion, string nroestudiantes, string nrodocentes)
    {
        ...
        DataRow insert = inst.procinsertEspaciosApropiacion(codproyecto, fun.convertFechaAño(fecharealizacion), nroestudiantes, nrodocentes , HttpContext.Current.Session["e"].ToString(), HttpContext.Current.Session["m"].ToString());
        if (insert != null)
        {
            ca = "true@";
        ...
        {
            ca = "Ocurrio un error al insertar datos de instrumento_Espacio@";
        }
        return ca;
    }
    ...
    [WebMethod(EnableSession = true)]
    public static string updateInstrumento(string codigoinstrumento, string fecharealizacion, string nroestudiantes, string nrodocentes)
    {
    ...
    [WebMethod(EnableSession = true)]
    public static string listarEspaciosApropiacion()
    {
        Institucion inst = new Institucion();
        Funciones fun = new Funciones();
        string ca = "";

        DataTable datos = inst.listarEspaciosApropiacion(HttpContext.Current.Session["CodAsesorEstraCoordinador"].ToString(), HttpContext.Current.Session["e"].ToString(), HttpContext.Current.Session["m"].ToString());
        if (datos != null && datos.Rows.Count > 0)
        {
            for (int i = 0; i < datos.Rows.Count; i++)
            {
    ...
    public static string loadInstrumento(string codigo)
    {
        ...
        DataRow dato = estra.loadEspacioApropiacion(codigo);
        if (dato != null)
        {
            ca += "true@";
            ...
            DataRow doce = doc.buscarDocenteEnEspacioApro(codigo);
    ...
    [WebMethod(EnableSession = true)]
    public static string loadSelectEspaciosApropiacion(string codigo)
    {
        string ca = "";
        Estrategias estra = new Estrategias();

        DataRow dato = estra.loadSelectEspaciosApropiacion(codigo);
        if (dato != null)
        {
            ca += "loadSelect@";
    ...
    public static string eliminar(string codigo)
    {
        ...
        estra.deleteEspacioApropiacion(codigo);
