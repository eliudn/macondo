...
public partial class confiespaciodeapropiacionevidenciasinsc : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            if (Session["codrol"] != null)
            {
                buscarUsuario();
                ObtenerGet();
                ddMunicipios(DropMunicipiosCargar, lblCodDepartamento.Text, lblCodEspacio.Text);
                ddMunicipios(DropMunicipiosBuscar, lblCodDepartamento.Text, lblCodEspacio.Text);
                //gvCargarEvidencias();
            }
            else
            {
    ...
    private void ddMunicipios(DropDownList drop, string coddepartamento, string codferiamunicipal)
    {
        drop.DataSource = estu.cargarmunicipiosxFerias(coddepartamento, codferiamunicipal);
    ...
    private void ObtenerGet()
    {
        ...
        lblCodDepartamento.Text = HttpUtility.UrlDecode(HttpContext.Current.Request.QueryString["coddep"]);
    }
    //Evidencias
    ...
    {
        Response.Redirect("confiespaciodeapropiacionint.aspx");
    }
    protected void btnActualizarDatosTutoria_Click(object sender, EventArgs e)
    {
        ...
        else if (variables[0] == "exito")
        {
            ...
            if (est.agregarArchivoRespositorioEspacioxInternacional(lblCodUsuario.Text, variables[2], variables[3], variables[4], variables[5], variables[7], Convert.ToInt32(variables[8]), fun.getFechaAñoHoraActual(), lblCodEspacio.Text, rbtActividades.SelectedValue))
            {
                //gvCargarEvidencias();
    ...
    private string[] subirArchivo(FileUpload trepador)
    {
        ...
        if (HttpContext.Current.Request.ApplicationPath.Remove(0, 1) == "")
            pathsinmap = "~/Ferias_internacionales/";
        else
            pathsinmap = "~/Ferias_internacionales/" + HttpContext.Current.Request.ApplicationPath.Remove(0, 1) + "/";
    ...
    {
        DataTable datos = est.cargarEvidenciasEspacioxInternacional(lblCodEspacio.Text, rbtActividadesBuscar.SelectedValue, lblCodUsuario.Text);
    ...
    private void borrarImagen(string cod)
    {
        ...
        DataRow dato = usu.buscarEvidenciaEspacioxInternacional(cod);
        if (usu.borrarEvidenciaEspacioxInternacional(cod))
        {
