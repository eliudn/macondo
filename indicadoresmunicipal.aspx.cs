...
public partial class indicadoresmunicipal : System.Web.UI.Page
{
    ...
    public static string realizarBusquedaloadreport(string coddepartamento, string codmuncipio, string codinstitucion, string codsede)
    {
        ...
        DataRow totalesturedestematicas = inst.totalesturedestematicas(codmuncipio);
        ...
        if (institucion != null && institucion.Rows.Count > 0)
        {
            ...
            {
                DataTable sedes = inst.cargarIndicadoresSedesInstitucion(institucion.Rows[i]["codigo"].ToString());
                ca += "<tr>";
                ...
                if (sedes != null && sedes.Rows.Count > 0)
                {
                    ...
                    {
                        DataRow totalxsede = inst.loadTotalesIndicadoresxSede(sedes.Rows[j]["codigosede"].ToString());
                        DataTable sedeestured = est.esturedtematicasxanio(coddepartamento, codmuncipio, institucion.Rows[i]["codigo"].ToString(), sedes.Rows[j]["codigosede"].ToString(), "2017");
                        double totalesturedes = ((double)Convert.ToInt32(sedeestured.Rows.Count) / (double)Convert.ToInt32(totalesturedestematicas["totalesturedes"].ToString()));
                        double pesofinal = ((double)Convert.ToInt32(totalxsede["totalesturedestematicas"].ToString()) / (double)Convert.ToInt32(totalesturedestematicas["totalesturedes"].ToString()));

                        //totalestuxMuni = totalestuxMuni + Convert.ToInt32(totalesturedestematicas["totalesturedes"].ToString());
                        totalllevaestu = totalllevaestu + Convert.ToInt32(sedeestured.Rows.Count);


                        metaporc = ((double)sedeestured.Rows.Count / (double)Convert.ToInt32(totalxsede["totalesturedestematicas"].ToString())) * 100;
                        ...
                        string stotal = totalesturedes.ToString("N2");
                        string spesofinal = pesofinal.ToString("N2");
                        if (totalesturedes > pesofinal)
                        {
                            stotal = spesofinal;
                        }
                        
                        sumpeso = sumpeso + totalesturedes;


                        double metaporcxMunicipio = 0;
                        metaporcxMunicipio = ((double)sedeestured.Rows.Count / (double)Convert.ToInt32(totalesturedestematicas["totalesturedes"].ToString())) * 100;
                        ...
                        ca += "<td style='text-align:center'>" + sedes.Rows[j]["totalesturedestematicas"].ToString() + "</td>";
                        ca += "<td style='text-align:center'>" + sedeestured.Rows.Count + "</td>";
                        ca += "<td style='text-align:center'>" + metaporc + "%</td>";
                        ca += "<td style='text-align:center'>" + stotal + " de " + spesofinal + "</td>";
                        //if (j == 0)
                        //{
                        ca += "<td  class='noExl center'><a href='indicadoresinstitucion.aspx?tipo=est&ins=" + institucion.Rows[i]["codigo"].ToString() + "&mun=" + codmuncipio + "&dep=" + coddepartamento + "&sed=" + sedes.Rows[j]["codigosede"].ToString() + "'><img src='images/detalles.png'>Ver</a></td>";
            ...
            ca += "@" + sumporc + "@" + totalllevaestu + "@" + totalesturedestematicas["totalesturedes"].ToString();
        }
