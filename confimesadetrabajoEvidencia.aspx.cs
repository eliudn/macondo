...
public partial class confimesadetrabajoEvidencia : System.Web.UI.Page
{
    ...
    public void obtenerGET()
    {
        ...
        lblCodGradoDocente.Text = HttpUtility.UrlDecode(HttpContext.Current.Request.QueryString["codgradodocente"]);
        lblCoord.Text = HttpUtility.UrlDecode(HttpContext.Current.Request.QueryString["coord"]);
    ...
    protected void btnActualizarDatosTutoria_Click(object sender, EventArgs e)
    {
        ...
        else if (variables[0] == "exito")
        {
            ...
            if (usu.agregarArchivoRespositorioMesaTrabajo(lblCodGradoDocente.Text, variables[2], variables[3], variables[4], variables[5], variables[7], Convert.ToInt32(variables[8]), fun.getFechaAñoHoraActual(), lblCodMesaTrabajo.Text))
                {
                    gvCargarEvidencias();
    ...
    private string[] subirArchivo(FileUpload trepador)
    {
        ...
        if (HttpContext.Current.Request.ApplicationPath.Remove(0, 1) == "")
            pathsinmap = "~/MesaTrabajo/";
        else
            pathsinmap = "~/MesaTrabajo/" + HttpContext.Current.Request.ApplicationPath.Remove(0, 1) + "/";
    ...
    {
        DataTable datos = est.cargarEvidenciasMesaTrabajo(lblCodMesaTrabajo.Text, lblCodGradoDocente.Text);
    ...
    {
        Estrategias usu = new Estrategias();
        DataRow dato = usu.buscarEvidenciaMesaTrabajo(cod);
        if (usu.borrarEvidenciaMesaTrabajo(cod))
        {
    ...
    {
        if(lblCoord.Text == "si")
            Response.Redirect("confimesadetrabajoeditarReporte.aspx");
        else
