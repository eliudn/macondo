...
public partial class estradoscontedigital_evi : System.Web.UI.Page
{
    ...
    protected void btnActualizarDatosTutoria_Click(object sender, EventArgs e)
    {
        ...
        else if (variables[0] == "exito")
        {
            ...
            if (inst.agregarArchivocontenidodigitalestrados(lblCodUsuario.Text, variables[2], variables[3], variables[4], variables[5], variables[7], Convert.ToInt32(variables[8]), fun.getFechaAñoHoraActual(), lblCodInstrumento.Text, rbtActividades.SelectedValue))
            {
                gvCargarEvidencias();
    ...
    {
        DataTable datos = inst.cargarEvidenciascontenidodigitalestrados(lblCodInstrumento.Text, lblCodUsuario.Text);
    ...
    private void borrarImagen(string cod)
    {
        ...
        DataRow dato = inst.buscarEvidenciacontenidodigitalestrados(cod);
        if (inst.borrarEvidenciacontenidodigitalestrados(cod))
        {
    ...
    {
        Response.Redirect("estradoscontedigital.aspx");
    }
