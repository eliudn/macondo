...
public partial class estraunopreestructuradoscoord : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            //else
            //{
            //    mostrarmensaje("error", "ERROR: Ud. No es coordinador de esta estrategía.");
            //}
    ...
    public static string cargarasesores()
    {
        ...
        DataTable datos = inst.listarAsesoresSeguimiento("1");
    ...
    [WebMethod(EnableSession = true)]
    public static string insertInstrumento(string codproyecto, string preestructurado)
    {
        ...
        DataRow insert = inst.procinsertIntrumentoPreestructurados(codproyecto, preestructurado, HttpContext.Current.Session["e"].ToString(), HttpContext.Current.Session["m"].ToString());
        if (insert != null)
        {
            ca = "true@";
    ...
    [WebMethod(EnableSession = true)]
    public static string updateInstrumento(string codigoinstrumento, string preestructurado)
    {
        ...
        if (inst.updateIntrumentoPreestructurados(codigoinstrumento, preestructurado))
        {
            ca = "true@";
        }
        else
    ...
    [WebMethod(EnableSession = true)]
    public static string listarPreesctructurados(string codasesorcoordinador)
    {
        Institucion inst = new Institucion();
        string ca = "";

        //<a class='btn btn-success' onclick=\"$('#table').hide(), $('#form').fadeIn(500),  loadSelectBitacoraSiete(" + datos.Rows[i]["codproyectosede"].ToString() + "), loadInstrumentosSeis(" + datos.Rows[i]["codigo"].ToString() + ") \">Editar</a>

        DataTable datos = inst.listarPreestructurados(codasesorcoordinador, HttpContext.Current.Session["e"].ToString(), HttpContext.Current.Session["m"].ToString());
        if (datos != null && datos.Rows.Count > 0)
        {
            for (int i = 0; i < datos.Rows.Count; i++)
            {
                ...
                ca += "<td>" + datos.Rows[i]["preestructurado"].ToString() + "</td>";
                ca += "<td>" + datos.Rows[i]["momento"].ToString() + "</td>";
                ca += "<td style='padding:5px;' ><a class='btn btn-primary' href='estraunopreestructuradosevi.aspx?cod=" + datos.Rows[i]["codigo"].ToString() + "&c=si'>Evidencias</a><br/></td>";
                ca += "</tr>";
            }
        }
        else
    ...
    public static string eliminar(string codigo)
    {
        ...
        estra.deletePreestructurados(codigo);
        estra.deletePreestructuradosEvidencia(codigo);
