...
public partial class estrag006reporte2 : System.Web.UI.Page
{
    ...
    public static string cargarListadoMaterialesAsesor()
    {
        ...
        DataTable datosmateriales = inst.cargarListadoMaterialessinCoordinador( "0", "1000");

        if (datosmateriales != null && datosmateriales.Rows.Count > 0)
        {
            DataTable datosCount = inst.cargarListadoMaterialessinCoordinadorCount();
            ...
            for (int i = 0; i < datosmateriales.Rows.Count; i++)
            {
                ca += "<tr>";
                ca += "<td>" + (i+1) + "</td>";
                ca += "<td>" + datosmateriales.Rows[i]["coordinador"].ToString() + "</td>";
                ...
                ca += "<td>" + datosmateriales.Rows[i]["nombrequienrecibe"].ToString() + "</td>";
                ca += "<td align=\"center\"><br /><a class='btn btn-success' onclick=\"$('#listTable').hide(), $('#formTable').fadeIn(500), loadInstrumento(" + datosmateriales.Rows[i]["codigo"].ToString() + ")\">Editar</a><br/ ><br /><a href=\"estrag006Evidencia.aspx?u=i&cod=" + datosmateriales.Rows[i]["codigo"].ToString() + "\" class=\"btn btn-primary\">Evidencias</a><br/ ><br /><br /></td>";
                ca += "</tr>";
            }
            ...
            //    if (pagint == (j + 1))
            //    {
            //        ca += " <span id='span" + (j + 1) + "' class=\"item current\" onclick='cargarListadoMateriales(\"" + (j + 1) + "\",\"10\")'>" + (j + 1) + "</span>";
            //    }
            //    else
            //    {
            //        ca += " <span id='span" + (j + 1) + "' class=\"item\" onclick='cargarListadoMateriales(\"" + (j + 1) + "\",\"10\")'>" + (j + 1) + "</span>";
            //    }
            //}
        }
        else
        {
            ca += "<tr><td colspan='11' align='center'>No se encontraron registros de materiales por parte del coordinador.</td></tr>";
        }
        ca += "@" + HttpContext.Current.Session["codrol"].ToString();
        return ca;
    }
    [WebMethod(EnableSession = true)]
    public static string loadSelectInstrumento(string codigo)
    {
        ...
        DataRow dato = estra.loadSelectInstrumentog006(codigo);
        if (dato != null)
        {
            ca += "loadSelect@";
    ...
    public static string insertInstrumento(string fechaentregamaterial, string codsede, string nombrequienrecibe)
    {
        ...
        DataRow insert = inst.procinsertIntrumento(fun.convertFechaAño(fechaentregamaterial), codsede, nombrequienrecibe, coord);
        if (insert != null)
        {
            ca = "true@";
    ...
    public static string loadInstrumento(string codigo)
    {
        ...
        DataRow datosinstrumento = inst.cargarDatosInstrumento(codigo);
    ...
    public static string loadMaterial(string codigoinstrumento,int total)
    {
        ...
        if (datos != null && datos.Rows.Count > 0)
        {
            ...
            for (int i = 0; i < datos.Rows.Count; i++)
            {
                ...
                ca += "<td align=\"center\" ><input type=\"text\" class=\"TextBox\" id =\"nombrematerial" + total + "\" name =\"nombrematerial" + total + "\" class=\"width-100\" value=\"" + datos.Rows[i]["nombrematerial"].ToString() + "\" style =\"width:250px;\" disabled></td>";
                ca += "<td align=\"center\"><input type=\"text\" class=\"TextBox\" id =\"cantidad" + total + "\" name =\"cantidad" + total + "\"  onkeypress=\"return valideKey(event);\" class=\"width-100\" value=\"" + datos.Rows[i]["cantidad"].ToString() + "\" style =\"width:40px;\"></td>";
    ...
    public static string deleteestrag006(string codigo)
    {
        ...
        if (inst.eliminarEncabezadoEntregaMaterial(codigo))
        {
    ...
    public static string eliminarEvidenciasEntregaMaterial(string codigo)
    {
        ...
        if (inst.eliminarEvidenciasEntregaMaterial(codigo))
        {
