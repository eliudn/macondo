...
public partial class estraunobitacoracincoevidencias : System.Web.UI.Page
{
    ...
    {
        lblCodestrabitacora5.Text = HttpUtility.UrlDecode(HttpContext.Current.Request.QueryString["codestrabitacora5"]);
    }
    //Evidencias
    ...
    {
        if (Session["codrol"].ToString() == "10" || Session["codrol"].ToString() == "18" || Session["codrol"].ToString() == "20")
            Response.Redirect("verestraunobitacoracinco.aspx?e=" + Session["e"].ToString() + "&m=" + Session["m"].ToString() + "&s=" + Session["s"].ToString());
        else
            Response.Redirect("estraunobitacoracinco.aspx?e=" + Session["e"].ToString() + "&m=" + Session["m"].ToString() + "&s=" + Session["s"].ToString());
    }
    protected void btnActualizarDatosTutoria_Click(object sender, EventArgs e)
    {
        ...
        else if (variables[0] == "exito")
        {
            ...
            if (usu.agregarArchivoRespositorioEstrategia1Asesorbitacora5(lblCodUsuario.Text, variables[2], variables[3], variables[4], variables[5], variables[7], Convert.ToInt32(variables[8]), fun.getFechaAñoHoraActual(), rbtActividades.SelectedValue, lblCodestrabitacora5.Text))
            {
                gvCargarEvidencias();
    ...
    {
        DataTable datos = est.cargarEvidenciasEstrategia1Asesorbitacora5(lblCodestrabitacora5.Text);
    ...
    {
        Estrategias usu = new Estrategias();
        DataRow dato = usu.buscarEvidenciaEstrategia1bitacora5(cod);
