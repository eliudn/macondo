...
public partial class estras003_estategia1Reporte : System.Web.UI.Page
{
    ...
    public static string cargarListadoS003(string page, string rows, string codasesorcoordinador)
    {
        string ca = "";
        ...
        DataTable datos = est.cargarInstrmentoS003_Estrategia1(codasesorcoordinador, Convert.ToString(offset), rows);

        if (datos != null && datos.Rows.Count > 0)
        {
            DataTable datosCount = est.cargarInstrmentoS003Count_Estrategia1(codasesorcoordinador);//Saber la cantidad de registros en la consulta
            ...
            for (int i = 0; i < datos.Rows.Count; i++)
            {
                ...
                ca += "<td align=\"center\"><br /><a class='btn btn-success' onclick=\"$('#lista').hide(), $('#form').fadeIn(500), loadInstrumento(" + datos.Rows[i]["codigo"].ToString() + "),loadSelect(" + datos.Rows[i]["codproyectosede"].ToString() + ")\">Ver</a><br /><br /></td>";
                ca += "</tr>";
            }
            ca += "@S3p4";
            for (int j = 0; j < val; j++)
