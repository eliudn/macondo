...
public partial class confiespaciodeapropiacionaprodoc : System.Web.UI.Page
{
    ...
    {
       lblCodApropiacion.Text = HttpUtility.UrlDecode(HttpContext.Current.Request.QueryString["cod"]);
       Session["CodApro"] = lblCodApropiacion.Text;
    }
    ...
    //cargar docentes
    [WebMethod(EnableSession = true)]
    public static string cargarDocentes(string codsede, string codanio, string municipio, string institucion, string sede)
    {
        string ca = "";
        ...
        if (datos != null && datos.Rows.Count > 0)
        {
            ...
            for (int i = 0; i < datos.Rows.Count; i++)
            {
                ...
                ca += "<td align='center'><input name=\"chkDocentes\" value=" + datos.Rows[i]["codgradodocente"].ToString() + " type=\"checkbox\"></td>";
                ca += "</tr>";
            }
        }
    ...
    public static string loadData(string dataID)
    {
        ...
        DataTable datos = ins.loadeventos(dataID);
        ...
        if (datos != null && datos.Rows.Count > 0)
        {
            ...
            for (int i = 0; i < datos.Rows.Count; i++)
            {
                ca += "<tr>";
                ca += "<td>"+(i+1)+"</td>";
                ca += "<td>" + datos.Rows[i]["tipoevento"].ToString() + "</td>";
                ca += "<td>" + fun.convertFechaAño(datos.Rows[i]["fecha"].ToString()) + "</td>";
                ca += "<td>" + datos.Rows[i]["horainicio"].ToString() + "</td>";
                ca += "<td>" + datos.Rows[i]["horafin"].ToString() + "</td>";
                ca += "<td align='center'><a class=\"btn btn-success\" onclick=\"fedit(" + datos.Rows[i]["codigo"].ToString() + ")\">Editar</a> <br><br> <a href='confiespaciodeapropiacionaprodocevi.aspx?cod=" + datos.Rows[i]["codigo"].ToString() + "' class=\"btn btn-primary\">Evidencias</a></br></br><a href='javascript:void(0)' onclick='eliminar(\"" + datos.Rows[i]["codigo"].ToString() + "\")' class='btn btn-danger'>Eliminar</a><br />";
                ca += "</tr>";
            }
        }
    ...
    [WebMethod(EnableSession = true)]
    public static string insertDocentes(string codencabezado, string codgradodocente)
    {

        Institucion inst = new Institucion();
        string ca = "";

        DataRow insert = inst.insertDocentesFeria(codencabezado, codgradodocente);
        if (insert != null)
        {
            ca += "true@";
            ca += insert["codigo"].ToString();
