...
public partial class estras004_espavirt_coordinador : System.Web.UI.Page
{
    ...
    [WebMethod(EnableSession = true)]
    public static string insertestras004(string redtematica, string nombresesion, string temasesion, string fechaelaboracion, string nombrerelator, string horasesion, string horasesionfinal, string acompanamiento, string herramientas, string evaluacionsesion)
    {
    ...
    [WebMethod(EnableSession = true)]
    public static string updateestras004(string codigoestrategia, string nombresesion, string temasesion, string fechaelaboracion, string nombrerelator, string horasesion, string horasesionfinal, string acompanamiento, string herramientas, string evaluacionsesion)
    {
        ...
        long update = inst.procupdateestras004_EspVirtual(codigoestrategia, nombresesion, temasesion, fechaelaboracion, nombrerelator, horasesion, horasesionfinal, acompanamiento, herramientas, evaluacionsesion,"");
        if (update != -1)
        {
            ca = "true@";
        }
    ...
    [WebMethod(EnableSession = true)]
    public static string cargarListadoMemoriaAsesor(string page, string codasesorcoordinador)
    {
        ...
        DataTable datos = estra.cargarListadoMemoriasS004_EspVirtuales(codasesorcoordinador, HttpContext.Current.Session["e"].ToString(), HttpContext.Current.Session["m"].ToString(), HttpContext.Current.Session["s"].ToString(), Convert.ToString(offset), Convert.ToString(rowsint));

        if (datos != null && datos.Rows.Count > 0)
        {
            DataTable datosCount = estra.cargarListadoMemoriasCount_EspVirtual(codasesorcoordinador, HttpContext.Current.Session["e"].ToString(), HttpContext.Current.Session["m"].ToString(), HttpContext.Current.Session["s"].ToString());//Saber la cantidad de registros en la consulta
            ...
            for (int i = 0; i < datos.Rows.Count; i++)
            {
                ...
                ca += "<td align=\"center\"><br /><a class='btn btn-success' onclick=\"$('#listTable').hide(), $('#formTable').fadeIn(500), loadInstrumento(" + datos.Rows[i]["codigo"].ToString() + ")\">Ver</a><br/ ><br /><a href=\"estras004_espavirtEvidencia.aspx?cod=" + datos.Rows[i]["codigo"].ToString() + "\" class=\"btn btn-primary\">Evidencias</a><br/ ><br /><a href=\"estrag001_estrategia4_ev.aspx?codinstrumentos004=" + datos.Rows[i]["codigo"].ToString() + "&codredtematicasede=" + datos.Rows[i]["codredtematicasede"].ToString() + " \" class=\"btn btn-success\">Asistencias</a><br/ ><br /></td>";
                ca += "</tr>";
            }
            ca += "@";
