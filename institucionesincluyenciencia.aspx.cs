...
public partial class institucionesincluyenciencia : System.Web.UI.Page
{
    ...
    public void obtenerGET()
    {
        ...
        subtitulo.Text = "Indicador: Instituciones educativas que incluyen ciencia en los énfasis formativos de sus PEI.";
        descripcion.Text = "";
  ...
  for (int i = 0; i < datos.Rows.Count; i++)
    ...
    [WebMethod(EnableSession = true)]
    public static string institucionesincluyenciencia_(string respuesta, string opc)
    {
        ...
        DataTable institucionesincluyenciencia = lb.detalleInstitucionesincluyenxRespuesta("", "", "", respuesta1, opc1);


        if (institucionesincluyenciencia != null && institucionesincluyenciencia.Rows.Count > 0)
        {
            int total = institucionesincluyenciencia.Rows.Count;
            ca = "lleno@";
            for (int i = 0; i < institucionesincluyenciencia.Rows.Count; i++)
            {
                var f = i + 1;
                ca += "<tr>";
                ca += "<td><b>" + f + ".</b> </td>";
                ca += "<td>" + institucionesincluyenciencia.Rows[i]["nombredepartamento"].ToString() + " </td>";
                ca += "<td>" + institucionesincluyenciencia.Rows[i]["nombremunicipio"].ToString() + " </td>";
                ca += "<td>" + institucionesincluyenciencia.Rows[i]["dane"].ToString() + " - " + institucionesincluyenciencia.Rows[i]["nombreins"].ToString() + " </td>";
                ca += "</tr>";
            }
            ca += "@" + total;
        }
    ...
    [WebMethod(EnableSession = true)]
    public static string realizarBusquedainstitucionesincluyenciencia_(string coddepartamento, string codmunicipio, string codinstitucion, string respuesta, string opc)
    {
        string ca = "";
        var lb = new LineaBase();

        DataTable institucionesincluyenciencia = lb.detalleInstitucionesincluyenxRespuesta(coddepartamento, codmunicipio, codinstitucion, respuesta, opc);
