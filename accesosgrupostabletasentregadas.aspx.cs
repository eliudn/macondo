...
public partial class Accesosgrupostabletasentregadas : System.Web.UI.Page
{
    ...
    public void obtenerGET()
    {
        ...
        subtitulo.Text = "Indicador: Acceso de los grupos de investigación a las tabletas entregadas por el Proyecto a las sedes educativas beneficiadas.";
        descripcion.Text = "";
  ...
  for (int i = 0; i < datos.Rows.Count; i++)
    ...
    [WebMethod(EnableSession = true)]
    public static string Accesosgrupostabletasentregadas_(string respuesta)
    {
        ...
        DataTable Accesosgrupostabletasentregadas = lb.detalleAccesostabletasGruposxFrecuencia("", "", "", "", respuesta1);


        if (Accesosgrupostabletasentregadas != null && Accesosgrupostabletasentregadas.Rows.Count > 0)
        {
            int total = Accesosgrupostabletasentregadas.Rows.Count;
            ca = "lleno@";
            for (int i = 0; i < Accesosgrupostabletasentregadas.Rows.Count; i++)
            {
                var f = i + 1;
                ca += "<tr>";
                ca += "<td><b>" + f + ".</b> </td>";
                ca += "<td>" + Accesosgrupostabletasentregadas.Rows[i]["nombredepartamento"].ToString() + " </td>";
                ca += "<td>" + Accesosgrupostabletasentregadas.Rows[i]["nombremunicipio"].ToString() + " </td>";
                ca += "<td>" + Accesosgrupostabletasentregadas.Rows[i]["dane"].ToString() + " - " + Accesosgrupostabletasentregadas.Rows[i]["nombreins"].ToString() + " </td>";
                ca += "<td>" + Accesosgrupostabletasentregadas.Rows[i]["danesede"].ToString() + " - " + Accesosgrupostabletasentregadas.Rows[i]["nombresede"].ToString() + " </td>";
                ca += "<td>" + Accesosgrupostabletasentregadas.Rows[i]["respuesta"].ToString() + " </td>";
                ca += "</tr>";
            }
            ca += "@" + total;
        }
    ...
    [WebMethod(EnableSession = true)]
    public static string realizarBusquedaAccesosgrupostabletasentregadas_(string coddepartamento, string codmunicipio, string codinstitucion, string codsede, string respuesta)
    {
        string ca = "";
        var lb = new LineaBase();

        DataTable Accesosgrupostabletasentregadas = lb.detalleAccesostabletasGruposxFrecuencia(coddepartamento, codmunicipio, codinstitucion, codsede, respuesta);
