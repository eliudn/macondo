...
public partial class estraunopreestructurados : System.Web.UI.Page
{
    ...
    [WebMethod(EnableSession = true)]
    public static string listarPreesctructurados()
    {
        Institucion inst = new Institucion();
        string ca = "";
        ...
        DataTable datos = inst.listarPreestructurados(HttpContext.Current.Session["CodAsesorEstraCoordinador"].ToString(), HttpContext.Current.Session["e"].ToString(), HttpContext.Current.Session["m"].ToString());
        if (datos != null && datos.Rows.Count > 0)
        {
            for (int i = 0; i < datos.Rows.Count; i++)
            {
                ...
                ca += "<td style='padding:5px;' ><a class='btn btn-primary' href='estraunopreestructuradosevi.aspx?cod=" + datos.Rows[i]["codigo"].ToString() + "'>Evidencias</a><br/><br/><a class='btn btn-danger' onclick='eliminar(" + datos.Rows[i]["codigo"].ToString() + ")'>Eliminar</a><br/></td>";
                ca += "</tr>";
            }
        }
        else
