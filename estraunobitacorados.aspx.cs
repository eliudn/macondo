...
public partial class estraunobitacorados : System.Web.UI.Page
{
    ...
    [WebMethod(EnableSession = true)]
    public static string loadBitacora2(string codproyecto)
    {
        string ca = "";

        Institucion inst = new Institucion();

        DataRow datosbitacora = inst.procloadBitacora2(codproyecto);
    ...
    [WebMethod(EnableSession = true)]
    public static string listarBitacoraDos()
    {
        Institucion inst = new Institucion();
        string ca = "";

        DataTable datos = inst.listarBitacoraDos(HttpContext.Current.Session["CodAsesorEstraCoordinador"].ToString());
        if (datos != null && datos.Rows.Count > 0)
        {
            for (int i = 0; i < datos.Rows.Count; i++)
            {
                ...
                ca += "<td style='padding:5px;' ><a class='btn btn-success' onclick=\"$('#table').hide(), $('#form').fadeIn(500), cargarBitacora2(" + datos.Rows[i]["codigo"].ToString() + "), loadSelectBitacoraDos(" + datos.Rows[i]["codProyecto"].ToString() + ")\">Editar</a><br/><br/><a href='estraunobitacoradosevi.aspx?codestrabitacora5=" + datos.Rows[i]["codigo"].ToString() + "' class='btn btn-primary'>Historial Bitácora</a><br /><br /><a class='btn btn-danger' onclick='eliminar(" + datos.Rows[i]["codigo"].ToString() + ")'>Eliminar</a><br/></td>";
                ca += "</tr>";
            }
        }
        else
        {
            ca += "<tr><td colspan='7'>No se encontraron memorias registradas por parte del asesor.</td></tr>";
        }
    ...
    public static string eliminar(string codigo)
    {
        ...
        estra.deleteBitacora2(codigo);
        estra.deletePreguntasBitacora2(codigo);
