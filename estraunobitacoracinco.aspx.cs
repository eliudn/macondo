...
public partial class estraunobitacoracinco : System.Web.UI.Page
{
    ...
    [WebMethod(EnableSession = true)]
    public static string listadobitacora5()
    {
        string ca = "";

        Estrategias estra = new Estrategias();

        DataTable datos = estra.listarBitacoraCincoSupervision(HttpContext.Current.Session["codEstraAsesorCoordinador"].ToString());
        if (datos != null && datos.Rows.Count > 0)
        {
            for (int i = 0; i < datos.Rows.Count; i++)
            {
                ...
                ca += "<td style='padding:5px;text-align:center' ><a class='btn btn-success' onclick=\"$('#table').hide(), $('#form').fadeIn(500),  loadSelectBitacoraCinco(" + datos.Rows[i]["codigo"].ToString() + "), loadBitacora(" + datos.Rows[i]["codigo"].ToString() + ") \">Ver</a><br /><br /><a href='estraunobitacoracincoevidencias.aspx?codestrabitacora5=" + datos.Rows[i]["codigo"].ToString() + "' class='btn btn-primary'>Historial bitácora</a><br /><br /><a class='btn btn-danger' onclick=\"deleteBitacoraCinco(" + datos.Rows[i]["codigo"].ToString() + ")\">Eliminar</a><br /></td>";
                ca += "</tr>";
            }
        }
        else
