...
public partial class accesosredestabletasentregadas : System.Web.UI.Page
{
    ...
    public void obtenerGET()
    {
        ...
        subtitulo.Text = "Indicador: Acceso de las redes temáticas a las tabletas entregadas por el Proyecto a las sedes educativas beneficiadas.";
        descripcion.Text = "";
  ...
  for (int i = 0; i < datos.Rows.Count; i++)
    ...
    [WebMethod(EnableSession = true)]
    public static string accesosredestabletasentregadas_(string respuesta)
    {
        ...
        DataTable accesosredestabletasentregadas = lb.detalleAccesostabletasRedesxFrecuencia("", "", "", "", respuesta1);


        if (accesosredestabletasentregadas != null && accesosredestabletasentregadas.Rows.Count > 0)
        {
            int total = accesosredestabletasentregadas.Rows.Count;
            ca = "lleno@";
            for (int i = 0; i < accesosredestabletasentregadas.Rows.Count; i++)
            {
                var f = i + 1;
                ca += "<tr>";
                ca += "<td><b>" + f + ".</b> </td>";
                ca += "<td>" + accesosredestabletasentregadas.Rows[i]["nombredepartamento"].ToString() + " </td>";
                ca += "<td>" + accesosredestabletasentregadas.Rows[i]["nombremunicipio"].ToString() + " </td>";
                ca += "<td>" + accesosredestabletasentregadas.Rows[i]["dane"].ToString() + " - " + accesosredestabletasentregadas.Rows[i]["nombreins"].ToString() + " </td>";
                ca += "<td>" + accesosredestabletasentregadas.Rows[i]["danesede"].ToString() + " - " + accesosredestabletasentregadas.Rows[i]["nombresede"].ToString() + " </td>";
                ca += "<td>" + accesosredestabletasentregadas.Rows[i]["respuesta"].ToString() + " </td>";
                ca += "</tr>";
            }
            ca += "@" + total;
        }
    ...
    [WebMethod(EnableSession = true)]
    public static string realizarBusquedaaccesosredestabletasentregadas_(string coddepartamento, string codmunicipio, string codinstitucion, string codsede, string respuesta)
    {
        string ca = "";
        var lb = new LineaBase();

        DataTable accesosredestabletasentregadas = lb.detalleAccesostabletasRedesxFrecuencia(coddepartamento, codmunicipio, codinstitucion, codsede, respuesta);
