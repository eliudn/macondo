...
public partial class estras004SedesReporte : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            DataRow dat = est.buscarCodEstrategiaxCoordinador(Session["identificacion"].ToString());

            if(dat != null)
            {
                Session["codestracoordinador"] = dat["codestracoordinador"].ToString();
            }
        }
    }
    public void obtenerGET()
    {
        ...
        Session["ac"] = HttpUtility.UrlDecode(HttpContext.Current.Request.QueryString["ac"]);
        ...
        if (Session["j"] != null )
        {
            lblJornada.Text = Session["j"].ToString();
        }
    ...
    //Listado de asesores por coordinador
    [WebMethod(EnableSession = true)]
    public static string cargarListadoAsesoresxCoordinador()
    {
        string ca = "";

        Institucion inst = new Institucion();
        Estrategias est = new Estrategias();
        DataTable datos = est.listarAsesores("2");
        //DataTable datos = inst.cargarListadoAsesoresxCoordinadorGI(HttpContext.Current.Session["codestracoordinador"].ToString(), codanio);
        ...
        if (datos != null && datos.Rows.Count > 0)
        {
            ...
            for (int i = 0; i < datos.Rows.Count; i++)
            {
                ca += "<option value='" + datos.Rows[i]["codigo"].ToString() + "'>" + datos.Rows[i]["asesor"].ToString() + "</option>";
            }
        }
    ...
    [WebMethod(EnableSession = true)]
    public static string cargarListadoMemoriaAsesor(string page, string rows, string codasesorcoordinador, string anio)
    {
        ...
        HttpContext.Current.Session["codasesorcoordinador"] = codasesorcoordinador;

        DataTable datos = estra.cargarListadoMemoriasS004SedesReporte(codasesorcoordinador, HttpContext.Current.Session["e"].ToString(), HttpContext.Current.Session["m"].ToString(), HttpContext.Current.Session["s"].ToString(), Convert.ToString(offset), rows, Convert.ToInt32(HttpContext.Current.Session["j"].ToString()), anio);

        if (datos != null && datos.Rows.Count > 0)
        {
            DataTable datosCount = estra.cargarListadoMemoriasCountSedesReporte(codasesorcoordinador, HttpContext.Current.Session["e"].ToString(), HttpContext.Current.Session["m"].ToString(), HttpContext.Current.Session["s"].ToString(), HttpContext.Current.Session["j"].ToString(), anio);//Saber la cantidad de registros en la consulta
            ...
            for (int i = 0; i < datos.Rows.Count; i++)
            {
                ...
                ca += "<td align=\"center\">" + datos.Rows[i]["momento"].ToString() + "</td>";
                ca += "<td align=\"center\">" + datos.Rows[i]["sesion"].ToString() + "</td>";
                ca += "<td align=\"center\">" + datos.Rows[i]["jornada"].ToString() + "</td>";
                ca += "<td align=\"center\"><br /><a class='btn btn-success' onclick=\"$('#listTable').hide(), $('#formTable').fadeIn(500), loadInstrumento(" + datos.Rows[i]["codigo"].ToString() + ")\">Ver</a><br/ ><br /><a href=\"estras004SedesEvidencia.aspx?cod=" + datos.Rows[i]["codigo"].ToString() + "&ac=" + codasesorcoordinador + "&a="+ anio + "\" class=\"btn btn-primary\">Evidencias</a><br/ ><br /><a href=\"estrag001reportes.aspx?cod=" + datos.Rows[i]["codigo"].ToString() + "&codsede=" + datos.Rows[i]["codsede"].ToString() + "\" class=\"btn btn-success\">Asistencia</a><br /><br /></td>";
                ca += "</tr>";
            }
            ca += "@";
            for (int j = 0; j < val; j++)
            {
                if (pagint == (j + 1))
                {
                    ca += " <span id='span" + (j + 1) + "' class=\"item current\" onclick='cargarListadoMemorias(\"" + (j + 1) + "\",\"10\"," + codasesorcoordinador + ")'>" + (j + 1) + "</span>";
                }
                else
                {
                    ca += " <span id='span" + (j + 1) + "' class=\"item\" onclick='cargarListadoMemorias(\"" + (j + 1) + "\",\"10\", " + codasesorcoordinador + ")'>" + (j + 1) + "</span>";
                }
            }
        }
        else
    ...
    //Listado de memorias por asesor Regreso
    [WebMethod(EnableSession = true)]
    public static string cargarListadoMemoriaAsesorBack(string page, string rows)
    {
        int pagint = Convert.ToInt32(page);
        int rowsint = Convert.ToInt32(rows);
        int offset = (pagint - 1) * rowsint;
        ...
        string ca = "";
        string codasesorcoordinador = "";
        Estrategias estra = new Estrategias();
        Funciones fun = new Funciones();

        if (HttpContext.Current.Session["ac"] != null && HttpContext.Current.Session["a"] != null)
        {
            HttpContext.Current.Session["codasesorcoordinador"] = HttpContext.Current.Session["ac"].ToString();
            codasesorcoordinador = HttpContext.Current.Session["codasesorcoordinador"].ToString();

            DataTable datos = estra.cargarListadoMemoriasS004SedesReporte(codasesorcoordinador, HttpContext.Current.Session["e"].ToString(), HttpContext.Current.Session["m"].ToString(), HttpContext.Current.Session["s"].ToString(), Convert.ToString(offset), rows, Convert.ToInt32(HttpContext.Current.Session["j"].ToString()), HttpContext.Current.Session["a"].ToString());

            if (datos != null && datos.Rows.Count > 0)
            {
                DataTable datosCount = estra.cargarListadoMemoriasCountSedesReporte(codasesorcoordinador, HttpContext.Current.Session["e"].ToString(), HttpContext.Current.Session["m"].ToString(), HttpContext.Current.Session["s"].ToString(), HttpContext.Current.Session["j"].ToString(), HttpContext.Current.Session["a"].ToString());//Saber la cantidad de registros en la consulta
                ...
                for (int i = 0; i < datos.Rows.Count; i++)
                {
                    ...
                    ca += "<td align=\"center\">" + datos.Rows[i]["sesion"].ToString() + "</td>";
                    ca += "<td align=\"center\">" + datos.Rows[i]["jornada"].ToString() + "</td>";
                    ca += "<td align=\"center\"><br /><a class='btn btn-success' onclick=\"$('#listTable').hide(), $('#formTable').fadeIn(500), loadInstrumento(" + datos.Rows[i]["codigo"].ToString() + ")\">Ver</a><br/ ><br /><a href=\"estras004SedesEvidencia.aspx?cod=" + datos.Rows[i]["codigo"].ToString() + "&ac=" + codasesorcoordinador + "&a=" + HttpContext.Current.Session["a"].ToString() + "\" class=\"btn btn-primary\">Evidencias</a><br/ ><br /><a href=\"estrag001reportes.aspx?cod=" + datos.Rows[i]["codigo"].ToString() + "&codsede=" + datos.Rows[i]["codsede"].ToString() + "\" class=\"btn btn-success\">Asistencia</a><br /><br /></td>";
                    ca += "</tr>";
                }
                ca += "@";
                for (int j = 0; j < val; j++)
    ...
    public static string insertestras004(string codsede, string nombresesion, string temasesion, string informacionadicional, string fechaelaboracion, string nombrerelator, string horasesion, string horasesionfinal, string aspectosdesarrollados, string conclusiones, string bibliografia, string desarrollo1, string desarrollo3, string actividadesytareas, string aspectos)
    {
        ...
        DataRow insert = inst.insertestras004Sedes(codsede, nombresesion, temasesion, informacionadicional, fun.convertFechaAño(fechaelaboracion), nombrerelator, horasesion, horasesionfinal, HttpContext.Current.Session["CodAsesorEstraCoordinador"].ToString(), aspectosdesarrollados, conclusiones, bibliografia, HttpContext.Current.Session["e"].ToString(), HttpContext.Current.Session["m"].ToString(), HttpContext.Current.Session["s"].ToString(), HttpContext.Current.Session["j"].ToString(), desarrollo1, desarrollo3, actividadesytareas, aspectos);

        if (insert != null)
        {
            ca = "true@";
            ca += insert["codigo"].ToString();
            HttpContext.Current.Session["codsede"] = codsede;
        }
        else
        {
    ...
    public static string loadestras004(string codigo)
    {
        ...
        DataRow datosinstrumento = inst.proloadestras004Sede(codigo, HttpContext.Current.Session["e"].ToString(), HttpContext.Current.Session["m"].ToString(), HttpContext.Current.Session["s"].ToString(), HttpContext.Current.Session["j"].ToString());
        ...
        if (datosinstrumento != null)
        {
            ...
            + "@" + datosinstrumento["nombresesion"].ToString()//resp[2]
            + "@" + datosinstrumento["temasesion"].ToString()//resp[3]
            + "@" + datosinstrumento["informacionadicional"].ToString()//resp[4]
            + "@" + fun.convertFechaDia(datosinstrumento["fechaelaboracion"].ToString())//resp[5]
            + "@" + datosinstrumento["nombrerelator"].ToString()//resp[6]
            + "@" + datosinstrumento["aspectosdesarrollados"].ToString()//resp[7]
            + "@" + datosinstrumento["conclusiones"].ToString()//resp[8]
            + "@" + datosinstrumento["bibliografia"].ToString()//resp[9]
            + "@" + datosinstrumento["desarrollo1"].ToString()//resp[10]
            + "@" + datosinstrumento["desarrollo3"].ToString()//resp[11]
            + "@" + datosinstrumento["actividadesytareas"].ToString()//resp[12]
            + "@" + datosinstrumento["aspectos"].ToString()//resp[13]
            + "@" + datosinstrumento["horasesion"].ToString()//resp[14]
            + "@" + datosinstrumento["horasesionfinal"].ToString();//resp[13]
    ...
    public static string loadSelectInstrumento(string codigo)
    {
        ...
        DataRow dato = estra.loadSelectInstrumentos004Sedes(codigo);
        if (dato != null)
        {
            ca += "loadSelect@";
    ...
    [WebMethod(EnableSession = true)]
    public static string updateestras004(string codigoestrategia, string nombresesion, string temasesion, string informacionadicional, string fechaelaboracion, string nombrerelator, string horasesion, string horasesionfinal, string aspectosdesarrollados, string conclusiones, string bibliografia,  string desarrollo1, string desarrollo3, string actividadesytareas, string aspectos)
    {
        ...
        long update = inst.procupdateestras004Sede(codigoestrategia, nombresesion, temasesion, informacionadicional, fechaelaboracion, nombrerelator, horasesion, horasesionfinal, aspectosdesarrollados, conclusiones, bibliografia,  desarrollo1, desarrollo3, actividadesytareas, aspectos);
        if (update != -1)
        {
            ca = "true@";
        }
    ...
    public static string deleteestras004(string codigo)
    {
        ...
        if (inst.eliminarMemoriaS004Sedes(codigo))
        {
    ...
    public static string deletePreguntass004(string codigoestrategia)
    {
        ...
        long delete = inst.procdeletePreguntass004Sede(codigoestrategia);
        if (delete != 1)
        {
            ca = "true@";
        }
    ...
    public static string insertPreguntas(string codigoestrategia, string nopregunta, string pregunta)
    {
        ...
        DataRow insert = inst.procinsertPreguntass004Sedes(codigoestrategia, nopregunta, pregunta);
        if (insert != null)
        {
            ca = "true@";
        }
    ...
    public static string cargarRedtematica(string codsede)
    {
        ...
        if(HttpContext.Current.Session["codrol"] != null)
        {
            ...
            {
                DataTable datos = inst.proccargarRedtematica(codsede,"");
...
//            //if (dato == null)
//            //{
//            if (HttpContext.Current.Session["codsede"] != null)
//            {
//                if (usu.agregarArchivoRespositorioEstrategia4AsesorRedTemaica(lblCodUsuario.Text, variables[2], variables[3], variables[4], variables[5], variables[7], Convert.ToInt32(variables[8]), fun.getFechaAñoHoraActual(), lblMomento.Text, lblSesion.Text, rbtActividades.SelectedValue, "4", HttpContext.Current.Session["codsede"].ToString()))
//                    {
//                        gvCargarEvidencias();
...
//        }
    ...
    {
        //DataTable datos = est.cargarEvidenciasEstrategia4AsesorRedTematica(lblMomento.Text, lblSesion.Text, "4", HttpContext.Current.Session["codsede"].ToString(), lblCodUsuario.Text);
