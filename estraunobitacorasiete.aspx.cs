...
public partial class estraunobitacorasiete : System.Web.UI.Page
{
    ...
    [WebMethod(EnableSession = true)]
    public static string listarBitacoraSiete()
    {
        Institucion inst = new Institucion();
        string ca = "";

        DataTable datos = inst.listarBitacoraSiete(HttpContext.Current.Session["CodAsesorEstraCoordinador"].ToString(), HttpContext.Current.Session["e"].ToString(), HttpContext.Current.Session["m"].ToString(), HttpContext.Current.Session["s"].ToString());
        if (datos != null && datos.Rows.Count > 0)
        {
            for (int i = 0; i < datos.Rows.Count; i++)
            {
                ...
                ca += "<td style='padding:5px;' ><a class='btn btn-success' onclick=\"$('#table').hide(), $('#form').fadeIn(500),  loadSelectBitacoraSiete(" + datos.Rows[i]["codproyectosede"].ToString() + "), loadInstrumentosSeis(" + datos.Rows[i]["codigo"].ToString() + ") \">Editar</a><br/><br/><a href='estraunobitacorasieteevi.aspx?codestrabitacora5=" + datos.Rows[i]["codigo"].ToString() + "' class='btn btn-primary'>Historial Bitácora</a><br /><br /><a class='btn btn-danger' onclick='eliminar(" + datos.Rows[i]["codigo"].ToString() + ")'>Eliminar</a><br/></td>";
                ca += "</tr>";
            }
        }
        else
