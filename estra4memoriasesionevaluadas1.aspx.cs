...
public partial class estra4memoriasesionevaluadas1 : System.Web.UI.Page
{
    ...
    public void obtenerGET()
    {
        ...
        Session["anio"] = HttpUtility.UrlDecode(HttpContext.Current.Request.QueryString["anio"]);
        ...
        if (Session["m"].ToString() != null)
        {
            if (Session["m"].ToString() == "1")
            {
                if (Session["s"].ToString() != null)
                {
                    ...
                    {
                        titulo.Text = "Desarrollo de las sesiones  para la estrategia de de conformación de la comunidad de práctica, aprendizaje, saber conocimiento y transformación del proyecto Ciclón";
                        subtitulo.Text = "Sesiones evaluadas";
    ...
    [WebMethod(EnableSession = true)]
    public static string estra4memoriassesio1_()
    {
        string ca = "";
        Estrategias est = new Estrategias();

        DataTable estra4memoriasesion1 = est.est_estra4instrumento_s004_redt_2016_Evidencias("", "", "", "", "1", "Formatos de evaluación", HttpContext.Current.Session["anio"].ToString());
        ...
        if (estra4memoriasesion1 != null && estra4memoriasesion1.Rows.Count > 0)
        {
            ...
            for (int i = 0; i < estra4memoriasesion1.Rows.Count; i++)
            {
                ...
                ca += "<td>" + estra4memoriasesion1.Rows[i]["nombrearchivo"].ToString() + " </td>";
                ca += "</tr>";
            }
            ca += "@" + total;
        }
    ...
    public static string realizarBusquedaestra4memoriasesion1_(string coddepartamento, string codmuncipio, string codinstitucion, string codsede)
    {
        ...
        DataTable estra4memoriasesion1 = est.est_estra4instrumento_s004_redt_2016_Evidencias(coddepartamento, codmuncipio, codinstitucion, codsede, HttpContext.Current.Session["s"].ToString(), "Formatos de evaluación", HttpContext.Current.Session["anio"].ToString());
     ...
     {
         Response.Redirect("estra4memorias004.aspx?m=1&s=" + HttpContext.Current.Session["s"].ToString() + "&e=4");
         //if (Session["m"].ToString() != null)
         //{
         //    if (Session["m"].ToString() == "1")
         //    {
