...
public partial class estracincoevidencias : System.Web.UI.Page
{
    ...
    {
        //lblSubActividad.Text = HttpUtility.UrlDecode(HttpContext.Current.Request.QueryString["a"]);
    ...
    protected void btnActualizarDatosTutoria_Click(object sender, EventArgs e)
    {
        ...
        else if (variables[0] == "exito")
        {
            ...
            if (usu.agregarArchivoRespositorioEstrategia5(lblCodUsuario.Text, variables[2], variables[3], variables[4], variables[5], variables[7], Convert.ToInt32(variables[8]), fun.getFechaAñoHoraActual(), lblSesion.Text, rbtActividades.SelectedValue, "5"))
            {
                gvCargarEvidencias();
    ...
    {
        DataTable datos = est.cargarEvidenciasEstrategia5(lblSesion.Text, lblCodUsuario.Text, "5");
    ...
    {
        Estrategias usu = new Estrategias();
        DataRow dato = usu.buscarEvidenciaEstrategia5Coord(cod);
        if (usu.borrarEvidenciaEstrategia5Coord(cod))
        {
    ...
    protected void imgDescargar_Click(object sender, ImageClickEventArgs e)
    {
        ...
        ImageButton btndetails = sender as ImageButton;
        GridViewRow gvrow = (GridViewRow)btndetails.NamingContainer;
        string filepath = Server.MapPath(HttpUtility.HtmlDecode(gvrow.Cells[9].Text));
        string filename = HttpUtility.HtmlDecode(gvrow.Cells[8].Text);
