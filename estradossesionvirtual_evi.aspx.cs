...
public partial class estradossesionvirtual_evi : System.Web.UI.Page
{
    ...
    protected void btnActualizarDatosTutoria_Click(object sender, EventArgs e)
    {
        ...
        else if (variables[0] == "exito")
        {
            ...
            if (usu.agregarArchivoSesionVirtualEstra2(lblCodUsuario.Text, variables[2], variables[3], variables[4], variables[5], variables[7], Convert.ToInt32(variables[8]), fun.getFechaAñoHoraActual(), rbtActividades.SelectedValue, Session["s"].ToString()))
                {
                    gvCargarEvidencias();
    ...
    {
        DataTable datos = est.cargarEvidenciasSesionVirtualEstra2(Session["s"].ToString(), lblCodUsuario.Text);
    ...
    {
        DataTable datos = est.cargarEvidenciasSesionVirtualEstra2I(Session["s"].ToString());
        GridEvidencias.DataSource = datos;
        GridEvidencias.DataBind();
    ...
    {
        Estrategias usu = new Estrategias();
        DataRow dato = usu.buscarEvidenciaSesionVirtualEstra2(cod);
        if (usu.borrarEvidenciaSesionVirtualEstra2(cod))
        {
