...
public partial class estras004Evidencia : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            if (Session["codrol"] != null)
            {
                ...
                if (Session["codrol"].ToString() == "12" || Session["codrol"].ToString() == "9")//Coordinador y Supervisor FUNTICS
                {
                    Panel.Visible = false;
                }
    ...
    {
        lblCodInstrumento.Text = HttpUtility.UrlDecode(HttpContext.Current.Request.QueryString["codinstrumentos004"]);
    ...
    protected void btnActualizarDatosTutoria_Click(object sender, EventArgs e)
    {
        ...
        else if (variables[0] == "exito")
        {
            ...
            if (usu.agregarArchivoRespositorioEstrategia4AsesorRedTemaica(lblCodUsuario.Text, variables[2], variables[3], variables[4], variables[5], variables[7], Convert.ToInt32(variables[8]), fun.getFechaAñoHoraActual(), Session["m"].ToString(), Session["s"].ToString(), rbtActividades.SelectedValue, "4", lblCodInstrumento.Text))
                {
                    gvCargarEvidencias();
    ...
    {
        DataTable datos = est.cargarEvidenciasEstrategia4AsesorRedTematica(lblCodInstrumento.Text, lblCodUsuario.Text, Session["s"].ToString());
    ...
    {
        Response.Redirect("estras004.aspx?e=" + Session["e"].ToString() + "&m=" + Session["m"].ToString() + "&s=" + Session["s"].ToString());
    }
    ...
    protected void dropListado_OnSelectedIndexChanged(object sender, EventArgs e)
    {
        ddListado(dropListado.SelectedValue);
    }

    private void ddListado(string actividad)
    {
        Estrategias est = new Estrategias();

        DataTable datos = est.cargarEvidenciasEstrategia4AsesorRedTematicaActiv(lblCodInstrumento.Text, lblCodUsuario.Text, actividad);
        GridEvidencias.DataSource = datos;
        GridEvidencias.DataBind();
